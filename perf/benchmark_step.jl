...
Logging.with_logger(Logging.NullLogger()) do
    ...
    sim = init_sim("Bomex"; prefix = "bm_step_Bomex", single_timestep = true)
    ...
    sim = init_sim("TRMM_LBA"; prefix = "bm_step_TRMM_LBA", single_timestep = true)
    (prob, alg, kwargs) = solve_args(sim)
    ODE.step!(integrator) # compile first
    b = BenchmarkTools.@benchmarkable ODE.step!($integrator)
    tfinal = integrator.sol.prob.tspan[2]
    ...
    sim = init_sim("Rico"; prefix = "bm_step_Rico", single_timestep = true)
    (prob, alg, kwargs) = solve_args(sim)
    ODE.step!(integrator) # compile first
    b = BenchmarkTools.@benchmarkable ODE.step!($integrator)
    tfinal = integrator.sol.prob.tspan[2]

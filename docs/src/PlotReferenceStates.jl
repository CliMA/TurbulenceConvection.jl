...
function export_ref_profile(case_name::String)
    ...
    aux_vars(FT) = (; ref_state = (ρ = FT(0), α = FT(0), p = FT(0)))
    ...
    α_c = vec(aux.cent.ref_state.α)
    ρ_f = vec(aux.face.ref_state.ρ)
    p_f = vec(aux.face.ref_state.p)
    α_f = vec(aux.face.ref_state.α)
    ...
    p3 = Plots.plot(α_c, zc ./ 1000; label = "centers")
    Plots.plot!(α_f, zf ./ 1000; label = "faces")
    Plots.plot!(size = (1000, 400))
    Plots.plot!(margin = 5 * Plots.mm)
    Plots.xlabel!("α_0")
    Plots.ylabel!("z (km)")

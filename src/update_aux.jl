function update_aux!(edmf, gm, grid, state, Case, param_set, TS)
    ...
    for i in 1:(up.n_updrafts)
        for k in real_center_indices(grid)
            ρa_cond = prog_up[i].ρarea[k] < ρ0_c[k]*edmf.minimum_area
            # @show k, ρa_cond
            aux_up[i].area[k]      = ρa_cond ? 0                    : prog_up[i].ρarea[k] / ρ0_c[k]
            # aux_up[i].area[k]      = prog_up[i].ρarea[k] / ρ0_c[k]
            aux_up[i].θ_liq_ice[k] = ρa_cond ? prog_gm.θ_liq_ice[k] : prog_up[i].ρaθ_liq_ice[k] / prog_up[i].ρarea[k]
            aux_up[i].q_tot[k]     = ρa_cond ? prog_gm.q_tot[k]     : prog_up[i].ρaq_tot[k] / prog_up[i].ρarea[k]
        end
        ρa_cond = prog_up[i].ρarea[kc_surf] < ρ0_c[kc_surf]*edmf.minimum_area
        # if ρa_cond
    ...
    for k in real_face_indices(grid)
        for i in 1:(up.n_updrafts)
            ...
            ρa_up_bcs = (; bottom = SetValue(ρ0_f[k]*edmf.area_surface_bc[i]), top = SetZeroGradient())
            a_up_f = interpc2f(aux_up[i].area, grid, k; a_up_bcs...)
            ρa_up_f = interpc2f(prog_up[i].ρarea, grid, k; ρa_up_bcs...)
            ρa_cond = a_up_f < edmf.minimum_area
            aux_up_f[i].w[k] = ρa_cond ? 0 : prog_up_f[i].ρaw[k] / ρa_up_f
        end
    end
    println("---")
    @show vec(prog_up[1].ρarea)[1:6]
    @show vec(prog_up_f[1].ρaw)[1:6]
    @show vec(aux_up_f[1].w)[1:6]

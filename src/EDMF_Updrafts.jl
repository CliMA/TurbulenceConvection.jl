...
function compute_nonequilibrium_moisture_tendencies!(
    ...
    @inbounds for i in 1:N_up
        ...
        @inbounds for k in real_center_indices(grid)
            ...
            mph = noneq_moisture_sources(param_set, nonequilibrium_moisture_scheme, moisture_sources_limiter, aux_up[i].area[k], ρ_c[k], aux_up[i].p[k], aux_up[i].T[k], Δt + ε, aux_up[i].ts[k], w[k],  aux_up[i].q_vap_sat_liq[k], aux_up[i].q_vap_sat_ice[k], aux_tc.dqvdt[k], aux_tc.dTdt[k], aux_up[i].τ_liq[k], aux_up[i].τ_ice[k]) # ts_LCL = nothing) # we currently don't store dqvdt for the updraft...


            if reweight_processes_for_grid
                mph = reweight_noneq_moisture_sources_for_grid(k, grid, param_set, thermo_params, aux_up[i], aux_up_f[i], mph, nonequilibrium_moisture_scheme, moisture_sources_limiter, Δt, ρ_c, p_c, w, aux_tc.dqvdt, aux_tc.dTdt; reweight_extrema_only = reweight_extrema_only)
            end
...
Computes tendencies to q_liq and q_ice due to...
"""
function compute_other_microphysics_tendencies!(state::State, edmf::EDMFModel, Δt::Real, param_set::APS, use_fallback_tendency_limiters::Bool)
    grid = Grid(state)
    thermo_params = TCP.thermodynamics_params(param_set)
    FT = float_type(state)
    N_up = n_updrafts(edmf)
    ...
    #     )[:cloud_base_ts] # cloud base, only keep the thermodynamic state part # deprecate for now
        ...
        w = aux_tc.w_up_c # reuse variable to save memory
        @. w = Ic(aux_up_f[i].w) # this one gets updated every time it is used
...
function compute_precipitation_formation_tendencies(
    ...
    @inbounds for i in 1:N_up
        if moisture_model isa NonEquilibriumMoisture
            # w::CC.Fields.Field = Ic.(aux_up_f[i].w) # fill correct type w/ interpolated w values
            w = aux_tc.w_up_c
            @. w = Ic(aux_up_f[i].w) # this one gets updated every time it's used [bc it's over all updrafts]
        end
        @inbounds for k in real_center_indices(grid)

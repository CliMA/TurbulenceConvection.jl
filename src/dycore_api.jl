...
""" Diagnostic fields for the host model """
diagnostics(state, fl) = getproperty(state.diagnostics, field_loc(fl))

center_diagnostics_grid_mean(state) = tendencies(state, CentField())
...
#= Diagnostic fields for TurbulenceConvection =#
diagnostics_tc(state, fl) = diagnostics(state, fl).turbconv

center_diagnostics_tc(state) = diagnostics_tc(state, CentField())

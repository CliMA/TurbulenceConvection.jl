function microphysics(
    ...
    @inbounds for k in real_center_indices(grid)
        ...
        aux_en.b_quad[k] = buoyancy_c(param_set, ρ_c[k], rho)
...
function quad_loop(en_thermo::SGSQuadrature, precip_model, vars, param_set, Δt::Real)
    ...
    env_len = 9
    src_len = 9
    i_ql, i_qi, i_T, i_cf, i_qt_sat, i_qt_unsat, i_T_sat, i_T_unsat, i_b = 1:env_len
    ...
    @inbounds for m_q in 1:quad_order
        ...
        for m_h in 1:quad_order
            ...
            inner_env[i_b] += b * weights[m_h] * sqpi_inv
...
function microphysics(
    ...
    @inbounds for k in real_center_indices(grid)
        if (
            ...
            aux_en.b_quad[k] = outer_env.b_quad
            ...
            tendencies_pr.q_sno[k] += mph.qs_tendency * aux_en.area[k]
            rho = TD.air_density(param_set, ts)
            aux_en.b_quad = buoyancy_c(param_set, ρ_c, rho)

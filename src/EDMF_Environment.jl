function microphysics(
    ...
    @inbounds for k in real_center_indices(grid)
        ...
        if edmf.moisture_model isa NonEquilibriumMoisture
            ...
            ###-###-### TRYNA DO SOME JAWN WIT NONEQ MICRO HERE, BUT ON EN NOT BULK/UP (move into loop if works)
            # tendencies_en = center_tendencies_environment(state) # do we need this? 
            # do we also need to update the gm tendencies in dycore?
            T_en = aux_en.T[k]
            ...
            aux_en.ql_tendency_noneq[k] = mph_neq.ql_tendency * aux_en.area[k] # How do I know these get used? gotta check in sum tendencies I guess? Is this even an existing tendency? And do we need to calculate the corresponding grid mean tendency?

...
function EnvironmentVariables(namelist, Gr::Grid)
    ...
    return EnvironmentVariables(;Gr,
        W,
        Area,
        QT,
        QL,
...
function update_EnvVar(self::EnvironmentThermodynamics,
    k, EnvVar::EnvironmentVariables,
    T, H, qt, ql,
    rho)
    EnvVar.T.values[k]   = T
...
function saturation_adjustment(self::EnvironmentThermodynamics, EnvVar::EnvironmentVariables)
  ...
  @inbounds for k in xrange(gw, self.Gr.nzg-gw)
        ...
        update_cloud_dry(self, k, EnvVar,
...
function sgs_quadrature(
  ...
  @inbounds for k in xrange(gw, self.Gr.nzg-gw)
        if (EnvVar.QTvar.values[k] > epsilon && EnvVar.Hvar.values[k] > epsilon && fabs(EnvVar.HQTcov.values[k]) > epsilon &&
            EnvVar.QT.values[k] > epsilon && sqrt(EnvVar.QTvar.values[k]) < EnvVar.QT.values[k])
            ...
            update_EnvVar(self, k, EnvVar, outer_env[i_T], outer_env[i_thl],

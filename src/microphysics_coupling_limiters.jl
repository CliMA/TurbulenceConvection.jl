...
@inline function resolve_S_S_addit(S_ql::FT, S_qi::FT, Δt_S::FT, S_ql_addit::FT, S_qi_addit::FT, Δt_S_addit::FT, Δt::FT) where {FT}
...
"""
function calculate_timestep_limited_sources(moisture_sources_limiter::NoMoistureSourcesLimiter, param_set::APS, area::FT, ρ::FT, p::FT, T::FT, w::FT, τ_liq::FT, τ_ice::FT, q_vap::FT, dqvdt::FT, dTdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, Δt::FT, ts::TD.ThermodynamicState) where {FT}

    S_ql = (q_vap - q_eq.liq) / τ_liq  
    S_qi = (q_vap - q_eq.ice) / τ_ice 

    return calculate_timestep_limited_sources(moisture_sources_limiter, param_set, area, ρ, p, T, w, τ_liq, τ_ice, q_vap, dqvdt, dTdt, q, q_eq, Δt, ts, S_ql, S_qi)
end

function calculate_timestep_limited_sources(moisture_sources_limiter::NoMoistureSourcesLimiter, param_set::APS, area::FT, ρ::FT, p::FT, T::FT, w::FT, τ_liq::FT, τ_ice::FT, q_vap::FT, dqvdt::FT, dTdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, Δt::FT, ts::TD.ThermodynamicState, S_ql::FT, S_qi::FT) where {FT}
...
"""
function calculate_timestep_limited_sources(moisture_sources_limiter::BasicMoistureSourcesLimiter, param_set::APS, area::FT, ρ::FT, p::FT, T::FT, w::FT, τ_liq::FT, τ_ice::FT, q_vap::FT, dqvdt::FT, dTdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, Δt::FT, ts::TD.ThermodynamicState) where {FT}
...
function calculate_timestep_limited_sources(moisture_sources_limiter::BasicMoistureSourcesLimiter, param_set::APS, area::FT, ρ::FT, p::FT, T::FT, w::FT, τ_liq::FT, τ_ice::FT, q_vap::FT, dqvdt::FT, dTdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, Δt::FT, ts::TD.ThermodynamicState, S_ql::FT, S_qi::FT) where {FT}
...
"""
function calculate_timestep_limited_sources(moisture_sources_limiter::MorrisonMilbrandt2015MoistureSourcesLimiter, param_set::APS, area::FT, ρ::FT, p::FT, T::FT, w::FT, τ_liq::FT, τ_ice::FT, q_vap::FT, dqvdt::FT, dTdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, Δt::FT, ts::TD.ThermodynamicState) where {FT}
    # this *shouldn't* need limiters the way it's defined but... lol we'll see...
    if iszero(Δt)
        return FT(0), FT(0)
    end
    S_ql, S_qi = morrison_milbrandt_2015_style(param_set, area, ρ, p, T, w, τ_liq, τ_ice, q_vap, dqvdt, dTdt, q, q_eq, Δt, ts; emit_warnings = false)
...
function calculate_timestep_limited_sources(moisture_sources_limiter::MorrisonMilbrandt2015ExponentialPartOnlyMoistureSourcesLimiter, param_set::APS, area::FT, ρ::FT, p::FT, T::FT, w::FT, τ_liq::FT, τ_ice::FT, q_vap::FT, dqvdt::FT, dTdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, Δt::FT, ts::TD.ThermodynamicState) where {FT}
    if iszero(Δt)
        return FT(0), FT(0)
    end
    ...
    S_ql, S_qi = morrison_milbrandt_2015_style_exponential_part_only(param_set, area, ρ, p, T, w, τ_liq, τ_ice, q_vap, dqvdt, dTdt, q, q_eq, Δt, ts; emit_warnings = false, fallback_to_standard_supersaturation_limiter = moisture_sources_limiter.fallback_to_standard_supersaturation_limiter)
    ...
    if !isfinite(S_ql) || !isfinite(S_qi)
        error("Morrison-Milbrandt 2015 (exponential part only) source calculations failed. Got S_ql = $S_ql; S_qi = $S_qi from inputs area = $area; ρ = $ρ; p = $p; T = $T; w = $w; τ_liq = $τ_liq; τ_ice = $τ_ice; q = $q; q_eq = $q_eq; Δt = $Δt; ts = $ts")
    end
    ...
    # if ((S_ql + S_qi)*Δt ≥ (q_vap+eps(FT))) && (!iszero(q_vap)) # consume all vapor (should never happen, really we should allow for liq and ice to contribute but all vapor gone is ridiculous, qv is like 2 orders of magnitude larger than ql, qi)
        @error("Morrison-Milbrandt 2015 (exponential part only) source calculations returned a total source greater than the available vapor. Got S_ql = $S_ql; S_qi = $S_qi from inputs area = $area; ρ = $ρ; p = $p; T = $T; w = $w; τ_liq = $τ_liq; τ_ice = $τ_ice; q_vap = $q_vap; q = $q; q_eq = $q_eq; Δt = $Δt; ts = $ts; dqvdt = $dqvdt; dTdt = $dTdt; dqsl_dT = $dqsl_dT; dqsi_dT = $dqsi_dT")
...
"""
@inline function get_qv_eq_point(q_eq::TD.PhasePartition, τ_liq::FT, τ_ice::FT; dδdt_no_S::FT = FT(0), Γ_l::FT = FT(1), Γ_i::FT = FT(1)) where {FT}    
    ...
    qv_eq = q_eq.liq * (τ_ice/(τ_liq + τ_ice)) + q_eq.ice * (τ_liq/(τ_liq + τ_ice)) + dδdt_no_S * (τ_liq * τ_ice)/(τ_liq + τ_ice) # Floating point safer form... [[ we don't combine bc first is x τ_ice, second is x τ_liq ]]


    if qv_eq == q_eq.liq # These are indistinguishable so we cannot have equilibrium
        error("Timescales τ_liq = $τ_liq and/or τ_ice = $τ_ice are too fast, qv_eq = q_eq.liq = $qv_eq (they are indistinguishable at this precision). Consider limiting the timescales or upgrading to BigFloats.")
    elseif qv_eq == q_eq.ice
        error("Timescales τ_liq = $τ_liq and/or τ_ice = $τ_ice are too fast, qv_eq = q_eq.ice = $qv_eq (they are indistinguishable at this precision). Consider limiting the timescales or upgrading to BigFloats.")
    end
...
@inline function get_δ_eq_point(q_eq::TD.PhasePartition, τ_liq::FT, τ_ice::FT; dδdt_no_S::FT = FT(0), Γ_l::FT = FT(1), Γ_i::FT = FT(1)) where {FT} # = get_qv_eq_point(q_eq, τ_liq, τ_ice; dδdt_no_S=dδdt_no_S, Γ_l=Γ_l, Γ_i=Γ_i) - q_eq.liq # this is the point where the vapor sources balance out, i.e. the supersaturation is zero
    ...
    if iszero(δi_eq)
        ...
        error("Timescales τ_liq = $τ_liq and/or τ_ice = $τ_ice are too fast, δi_eq = 0 (they are indistinguishable at this precision). Consider limiting the timescales or upgrading to BigFloats.")
    end
    if iszero(δ_eq)
        error("Timescales τ_liq = $τ_liq and/or τ_ice = $τ_ice are too fast, δ_eq = 0 (they are indistinguishable at this precision). Consider limiting the timescales or upgrading to BigFloats.")
    end
...
"""
function calculate_next_standard_milestone_time_given_milestones(S_ql::FT, S_qi::FT, dδdt::FT, δ_0::FT, δ_0i::FT, δ_eq::FT, δi_eq::FT, q_liq::FT, q_ice::FT; dδdt_is_full_tendency::Bool=false, Γ_l::FT = FT(1), Γ_i::FT = FT(1), at_δ_eq_point::Bool = false, allow_δ_eq_point::Bool = true) where {FT}
...
calculate_next_standard_milestone_time_given_milestones(S_ql::FT, S_qi::FT, dδdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, δ_eq::FT, δi_eq::FT; dδdt_is_full_tendency::Bool=false, Γ_l::FT = FT(1), Γ_i::FT = FT(1), at_δ_eq_point::Bool = false, allow_δ_eq_point::Bool = true) where {FT} = calculate_next_standard_milestone_time_given_milestones(S_ql, S_qi, dδdt, TD.vapor_specific_humidity(q), δ_eq, δi_eq, q.liq, q.ice, q_eq; dδdt_is_full_tendency = dδdt_is_full_tendency, Γ_l = Γ_l, Γ_i = Γ_i, at_δ_eq_point = at_δ_eq_point, allow_δ_eq_point = allow_δ_eq_point)
...
"""
function calculate_next_standard_milestone_time(regime::AbstractSaturationRegime, q_eq::TD.PhasePartition, q_liq::FT, q_ice::FT, δ_0::FT, δ_0i::FT, below_freezing::Bool, τ_liq::FT, τ_ice::FT; dδdt_no_S::FT = FT(0), Γ_l::FT = FT(1), Γ_i::FT = FT(1), at_δ_eq_point::Bool = false, allow_δ_eq_point::Bool = true) where {FT}
    ...
    return calculate_next_standard_milestone_time(q_eq, q_liq, q_ice, δ_0, δ_0i, δ_eq, δi_eq, S_ql, S_qi; dδdt = dδdt, dδdt_is_full_tendency = true, Γ_l = Γ_l, Γ_i = Γ_i, at_δ_eq_point = at_δ_eq_point, allow_δ_eq_point = allow_δ_eq_point) # dδdt_is_full_tendency is a full tendency here as constructed by definition since it started w/o S
end
...
function get_new_saturation_regime_type_from_milestone(milestone::MilestoneType, regime::AbstractSaturationRegime, old_δ_0::FT, old_δ_0i::FT) where {FT}
    regime_type = 
        ...
        elseif milestone == AtSaturationOverLiquidMilestone # liq sat, above freezing so that's going to Subsat
            if is_below_freezing(regime)
                ...
                elseif (regime isa Subsaturated)
                    if !iszero(old_δ_0i)
                        error("Below freezing, we shouldn't be able to hit liq sat from Subsaturated unless δ_0i was zero")
                    else
                        Supersaturated
                    end
                else 
                ...
                if (regime isa Supersaturated)
                    if !iszero(old_δ_0i)
                        error("Above freezing, we shouldn't be able to hit liq sat from Supersaturated unless δ_0i was zero")
                    else
                        Subsaturated
                    end
                elseif regime isa WBF
        ...
        elseif milestone == AtSaturationOverIceMilestone # ice sat, above freezing so that's going to Supersat
            if is_below_freezing(regime)
                if (regime isa Supersaturated)
                    if !iszero(old_δ_0)
                        error("Above freezing, we shouldn't be able to hit ice sat from Supersaturated unless δ_0 was zero")
                    else
                        Subsaturated
                    end
                elseif regime isa WBF
                ...
                elseif regime isa Subsaturated
                    if !iszero(old_δ_0)
                        error("Above freezing, we shouldn't be able to hit ice sat from Subsaturated unless δ_0 was zero")
                    else
                        Supersaturated
                    end
                else
...
get_new_saturation_regime_enum_type_from_milestone(milestone::MilestoneType, regime::AbstractSaturationRegime, old_δ_0::FT, old_δ_0i::FT) where {FT} = get_saturation_regime_enum_type(get_new_saturation_regime_type_from_milestone(milestone, regime, old_δ_0, old_δ_0i))
...
function calculate_timestep_limited_sources(moisture_sources_limiter::StandardSupersaturationMoistureSourcesLimiter, param_set::APS, area::FT, ρ::FT, p::FT, T::FT, w::FT, τ_liq::FT, τ_ice::FT, q_vap::FT, dqvdt::FT, dTdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, Δt::FT, ts::TD.ThermodynamicState) where {FT}
    δ_0 = q_vap - q_eq.liq
    δ_0i = q_vap - q_eq.ice

    use_fix = true # same as MM2015_EPA default, but this might be bad at high T, maybe set to false later

    return standard_supersaturation_sources(moisture_sources_limiter, param_set, area, ρ, p, T, w, τ_liq, τ_ice, δ_0, δ_0i, dqvdt, dTdt, q, q_eq, Δt, ts; use_fix = use_fix)
end

function standard_supersaturation_sources(moisture_sources_limiter::StandardSupersaturationMoistureSourcesLimiter, param_set::APS, area::FT, ρ::FT, p::FT, T::FT, w::FT, τ_liq::FT, τ_ice::FT, δ_0::FT, δ_0i::FT, dqvdt::FT, dTdt::FT, q::TD.PhasePartition, q_eq::TD.PhasePartition, Δt::FT, ts::TD.ThermodynamicState; use_fix::Bool = true) where {FT}

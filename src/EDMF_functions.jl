...
function compute_up_tendencies!(edmf::EDMFModel, grid::Grid, state::State, param_set::APS, surf::SurfaceBase)
    ...
    @inbounds for i in 1:N_up
        ...
        elseif edmf.entrainment_type isa TotalRateEntrModel
            @. tends_ρarea = -∇c(wvec(LBF(Ic(w_up) * ρarea))) + entr_rate_inv_s - detr_rate_inv_s
        end
        ...
        elseif edmf.entrainment_type isa TotalRateEntrModel
            @. tends_ρaθ_liq_ice =
                -∇c(wvec(LBF(Ic(w_up) * ρaθ_liq_ice))) + (entr_rate_inv_s * θ_liq_ice_en) -
                (detr_rate_inv_s * θ_liq_ice_up) + (ρ_c * θ_liq_ice_tendency_precip_formation)

            @. tends_ρaq_tot =
                -∇c(wvec(LBF(Ic(w_up) * ρaq_tot))) + (entr_rate_inv_s * q_tot_en) - (detr_rate_inv_s * q_tot_up) +
    ...
    @inbounds for i in 1:N_up
        ...
        elseif edmf.entrainment_type isa TotalRateEntrModel
            @. tends_ρaw +=
                (ᶠinterp_a(entr_rate_inv_s) * w_en - ᶠinterp_a(detr_rate_inv_s) * w_up) +
...
function compute_en_tendencies!(
    ...
    @inbounds for i in 1:N_up
        ...
        elseif edmf.entrainment_type isa TotalRateEntrModel
            @. D_env += Int(a_up > min_area) * entr_rate_inv_s
        end
    end

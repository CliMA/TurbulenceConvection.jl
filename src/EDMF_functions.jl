...
function compute_sgs_flux!(edmf::EDMFModel, grid::Grid, state::State, surf::SurfaceBase, param_set::APS)
    ...
    @. h_tot_en = (
        TD.total_energy(param_set, ts_en, 0.5 * (prog_gm.u^2 + prog_gm.v^2 + Ic(w_en)^2), grid.zc.z * g) +
        TD.gas_constant_air(param_set, ts_en) * aux_en.T
    )
    ...
    @inbounds for i in 1:N_up
        ...
        @. h_tot_up_i = (
            ...
            TD.gas_constant_air(param_set, ts_up_i) * aux_up_i.T
        )

...
function compute_sgs_flux!(edmf::EDMFModel, grid::Grid, state::State, surf::SurfaceBase)
    ...
    @. massflux_h = ρ_f * Ifae(a_en) * (w_en - w_gm) * (If(θ_liq_ice_en) - If(θ_liq_ice_gm))
    ...
    @inbounds for i in 1:N_up
        ...
        @. aux_up_f[i].massflux = ρ_f * Ifau(a_up) * (w_up - w_gm)
        # We know that, since W = 0 at z = 0, m = 0 also, and
        # therefore θ_liq_ice / q_tot values do not matter
        @. massflux_h += ρ_f * (Ifau(a_up) * (w_up - w_gm) * (If(θ_liq_ice_up) - If(θ_liq_ice_gm)))
        @. massflux_qt += ρ_f * (Ifau(a_up) * (w_up - w_gm) * (If(q_tot_up) - If(q_tot_gm)))
    end
    massflux[kf_surf] = 0

...
function io_dictionary_aux(edmf) # added EDMF as an argument so we can have things in the output without having to rely on them being in the model at runtime...
    ...
    io_dict = Dict{String, DT}(
        ...
        "env_CAPE" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_environment(state).CAPE),
        "up_CAPE" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_bulk(state).CAPE),
        ...
        # # SHOC diagnostic outputs - all internals for debugging
        # "shoc_N2" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.N2),
        # "shoc_pblh" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pblh),
        # "shoc_l_inf" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.l_inf),
        # "shoc_mixing_length" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.mixing_length),
        # "shoc_wstar" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.wstar),
        # "shoc_tscale" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.tscale),
        # "shoc_tke_eff" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.tke_eff),
        # "shoc_w2" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.w2),
        # "shoc_w3" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.w3),
        # "shoc_thl_var" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.thl_var),
        # "shoc_qt_var" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.qt_var),
        # "shoc_thl_qt_cov" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.thl_qt_cov),
        # "shoc_wthl_sec" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.wthl_sec),
        # "shoc_wqw_sec" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.wqw_sec),
        # "shoc_Km" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.Km),
        # "shoc_Kh" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.Kh),
        # "shoc_cloud_fraction" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.cloud_fraction),
        # "shoc_ql_mean" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.ql_mean),
        # "shoc_qi_mean" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.qi_mean),
        # "shoc_qc_total" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.qc_total),
        # "shoc_wqls" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.wqls),
        # "shoc_wthv_sec" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.wthv_sec),
        # "shoc_ql_var" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.ql_var),
        # "shoc_pdf_qt_mean" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_qt_mean),
        # "shoc_pdf_thl_mean" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_thl_mean),
        # "shoc_pdf_qt_var_input" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_qt_var_input),
        # "shoc_pdf_thl_var_input" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_thl_var_input),
        # "shoc_pdf_thl_qt_cov_input" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_thl_qt_cov_input),
        # "shoc_pdf_thl1_1" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_thl1_1),
        # "shoc_pdf_thl1_2" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_thl1_2),
        # "shoc_pdf_qw1_1" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_qw1_1),
        # "shoc_pdf_qw1_2" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_qw1_2),
        # "shoc_pdf_w1_1" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_w1_1),
        # "shoc_pdf_w1_2" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_w1_2),
        # "shoc_pdf_area_frac" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_area_frac),
        # "shoc_pdf_qs1" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_qs1),
        # "shoc_pdf_qs2" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_qs2),
        # "shoc_pdf_s1_signed" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_s1_signed),
        # "shoc_pdf_s2_signed" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_s2_signed),
        # "shoc_pdf_qsat_mean" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_qsat_mean),
        
        # # get_qc_stats_from_moments outputs
        # "qc_stats_ql" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.ql_from_moments),
        # "qc_stats_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.qi_from_moments),
        # "qc_stats_w_ql" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.w_ql_from_moments),
        # "qc_stats_w_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.w_qi_from_moments),
        # "qc_stats_cov_ql_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.cov_ql_qt_from_moments),
        # "qc_stats_cov_ql_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.cov_ql_h_from_moments),
        
        # # Standard deviations of qt in cloudy regions
        # "qc_stats_ql_qt_sd" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.ql_qt_sd),
        # "qc_stats_qi_qt_sd" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.qi_qt_sd),
        # "qc_stats_qc_mean_pdf" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.qc_mean_pdf),
        # "qc_stats_w_qc_cov_pdf" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.w_qc_cov_pdf),
        # "qc_stats_ql_pdf" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.ql_pdf),
        # "qc_stats_qi_pdf" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.qi_pdf),
        
        # # Intermediate diagnostics for debugging w_qi_cov spike
        # "qc_stats_pdf_cloud_frac" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_cloud_frac),
        # "qc_stats_pdf_alpha" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.pdf_alpha),
        # "qc_stats_phase_frac_liq" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.phase_frac_liq),
        # "qc_stats_phase_frac_ice" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.phase_frac_ice),
        # "qc_stats_flux_scale_liq" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.flux_scale_liq),
        # "qc_stats_flux_scale_ice" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.flux_scale_ice),
        # "qc_stats_w_ql_pot" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.w_ql_pot),
        # "qc_stats_w_qi_pot" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.w_qi_pot),

        # # cSigma correlation matrix outputs
        # "csigma_corr_w_ql" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_w_ql),
        # "csigma_corr_w_qr" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_w_qr),
        # "csigma_corr_w_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_w_qi),
        # "csigma_corr_w_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_w_qs),
        # "csigma_corr_w_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_w_qt),
        # "csigma_corr_w_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_w_h),
        # "csigma_corr_ql_qr" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_ql_qr),
        # "csigma_corr_ql_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_ql_qi),
        # "csigma_corr_ql_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_ql_qs),
        # "csigma_corr_ql_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_ql_qt),
        # "csigma_corr_ql_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_ql_h),
        # "csigma_corr_qr_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qr_qi),
        # "csigma_corr_qr_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qr_qs),
        # "csigma_corr_qr_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qr_qt),
        # "csigma_corr_qr_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qr_h),
        # "csigma_corr_qi_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qi_qs),
        # "csigma_corr_qi_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qi_qt),
        # "csigma_corr_qi_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qi_h),
        # "csigma_corr_qs_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qs_qt),
        # "csigma_corr_qs_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qs_h),
        # "csigma_corr_qt_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_corr_qt_h),

        # # cSigma covariance matrix outputs (diagonal and off-diagonal elements)
        # "csigma_cov_w" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_w),
        # "csigma_cov_ql" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_ql),
        # "csigma_cov_qr" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qr),
        # "csigma_cov_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qi),
        # "csigma_cov_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qs),
        # "csigma_cov_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qt),
        # "csigma_cov_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_h),
        # "csigma_cov_w_ql" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_w_ql),
        # "csigma_cov_w_qr" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_w_qr),
        # "csigma_cov_w_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_w_qi),
        # "csigma_cov_w_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_w_qs),
        # "csigma_cov_w_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_w_qt),
        # "csigma_cov_w_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_w_h),
        # "csigma_cov_ql_qr" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_ql_qr),
        # "csigma_cov_ql_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_ql_qi),
        # "csigma_cov_ql_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_ql_qs),
        # "csigma_cov_ql_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_ql_qt),
        # "csigma_cov_ql_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_ql_h),
        # "csigma_cov_qr_qi" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qr_qi),
        # "csigma_cov_qr_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qr_qs),
        # "csigma_cov_qr_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qr_qt),
        # "csigma_cov_qr_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qr_h),
        # "csigma_cov_qi_qs" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qi_qs),
        # "csigma_cov_qi_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qi_qt),
        # "csigma_cov_qi_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qi_h),
        # "csigma_cov_qs_qt" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qs_qt),
        # "csigma_cov_qs_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qs_h),
        # "csigma_cov_qt_h" => (; dims = ("zc", "t"), group = "profiles", field = state -> center_aux_turbconv(state).shoc.csigma_cov_qt_h),

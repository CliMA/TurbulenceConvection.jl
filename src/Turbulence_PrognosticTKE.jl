    ...
    @inbounds for k in real_center_indices(grid)
        if edmf.UpdVar.Area.bulkvalues[k] > 0.0
            @inbounds for i in 1:(edmf.n_updrafts)
                ...
                mean_entr_sc[k] += up_prog[i].area[k] * edmf.entr_sc[i, k] / a_up_bulk[k]
                mean_detr_sc[k] += up_prog[i].area[k] * edmf.detr_sc[i, k] / a_up_bulk[k]
                mean_asp_ratio[k] += up_prog[i].area[k] * edmf.asp_ratio[i, k] / a_up_bulk[k]
                mean_frac_turb_entr[k] += up_prog[i].area[k] * edmf.frac_turb_entr[i, k] / a_up_bulk[k]
                mean_horiz_K_eddy[k] += up_prog[i].area[k] * edmf.horiz_K_eddy[i, k] / a_up_bulk[k]
                mean_sorting_function[k] += up_prog[i].area[k] * edmf.sorting_function[i, k] / a_up_bulk[k]
                mean_b_mix[k] += up_prog[i].area[k] * edmf.b_mix[i, k] / a_up_bulk[k]
            end
        end
    end
...
function compute_gm_tendencies!(edmf::EDMF_PrognosticTKE, grid, state, Case, gm, TS)
    ...
    @inbounds for i in 1:(up.n_updrafts)
        ...
        @inbounds for k in real_face_indices(grid)
            a_up = interpc2f(up_prog[i].area, grid, k; a_up_bcs...)
...
function reset_surface_covariance(edmf::EDMF_PrognosticTKE, grid, state, GMV, Case::CasesBase)
    ...
    get_GMV_CoVar(edmf, grid, state, up.Area, up.W, up.W, en.W, en.W, en.TKE, gm.W.values, gm.W.values, gm.TKE.values)
    ...
    if is_tke
        @inbounds for k in real_center_indices(grid)
            ...
            @inbounds for i in 1:(edmf.n_updrafts)
                ...
                Δϕ = interpf2c(Δϕ_dual, grid, k)
                Δψ = interpf2c(Δψ_dual, grid, k)
                gmv_covar[k] += tke_factor * up_prog[i].area[k] * Δϕ * Δψ
            end
        end
    else
...
function compute_updraft_tendencies(edmf::EDMF_PrognosticTKE, grid, state, gm::GridMeanVariables, TS::TimeStepping)
    ...
    @inbounds for k in real_center_indices(grid)
        @inbounds for i in 1:(up.n_updrafts)
            ...
            adv = upwind_advection_area(ρ_0_c, up_prog[i].area, w_up[i, :], grid, k)

            a_up_c = up_prog[i].area[k]
    ...
    @inbounds for k in real_center_indices(grid)
        @inbounds for i in 1:(up.n_updrafts)
            w_up_c = interpf2c(w_up, grid, k, i)
            m_k = (ρ_0_c[k] * up_prog[i].area[k] * w_up_c)

            adv = upwind_advection_scalar(ρ_0_c, up_prog[i].area, w_up[i, :], up.H.values[i, :], grid, k)
            ...
            adv = upwind_advection_scalar(ρ_0_c, up_prog[i].area, w_up[i, :], up.QT.values[i, :], grid, k)
    ...
    @inbounds for k in real_face_indices(grid)
        ...
        @inbounds for i in 1:(up.n_updrafts)
            a_up_bcs = (; bottom = SetValue(edmf.area_surface_bc[i]), top = SetZeroGradient())
            a_k = interpc2f(up_prog[i].area, grid, k; a_up_bcs...)
            ...
            adv = upwind_advection_velocity(ρ_0_f, up_prog[i].area, w_up[i, :], grid, k; a_up_bcs)
...
function update_updraft(edmf::EDMF_PrognosticTKE, grid, state, gm::GridMeanVariables, TS::TimeStepping)
    ...
    @inbounds for k in real_center_indices(grid)
        @inbounds for i in 1:(up.n_updrafts)
            ...
            if a_up_new[i, k] >= edmf.minimum_area
                up.H.new[i, k] =
                    (ρ_0_c[k] * up_prog[i].area[k] * up.H.values[i, k] + Δt * up.H.tendencies[i, k]) /
                    (ρ_0_c[k] * a_up_new[i, k])

                up.QT.new[i, k] = max(
                    (ρ_0_c[k] * up_prog[i].area[k] * up.QT.values[i, k] + Δt * up.QT.tendencies[i, k]) /
                    (ρ_0_c[k] * a_up_new[i, k]),
                    0.0,
                )
    ...
    if is_tke
        @inbounds for k in real_center_indices(grid)
            ...
            @inbounds for i in 1:(edmf.n_updrafts)
                ...
                Covar.interdomain[k] += tke_factor * up_prog[i].area[k] * (1.0 - up_prog[i].area[k]) * Δϕ * Δψ
            end
        end
    else
        @inbounds for k in real_center_indices(grid)
...
function compute_covariance_detr(edmf::EDMF_PrognosticTKE, grid, state, Covar::EnvironmentVariable_2m)
    ...
    @inbounds for k in real_center_indices(grid)
        ...
        @inbounds for i in 1:(up.n_updrafts)
            ...
            Covar.detr_loss[k] += up_prog[i].area[k] * abs(w_up_c) * edmf.entr_sc[i, k]
        end
...
function en_diffusion_tendencies(grid::Grid, state, TS, covar, n_updrafts)
    ...
    @inbounds for k in real_center_indices(grid)
        ae[k] = 1 .- sum(ntuple(i -> up_prog[i].area[k], n_updrafts))
    end
...
function GMV_third_m(
    ...
    @inbounds for k in real_center_indices(grid)
        ...
        @inbounds for i in 1:(up.n_updrafts)
            ...
            GMVv_ += up_prog[i].area[k] * mean_up
        end
        ...
        @inbounds for i in 1:(up.n_updrafts)
            mean_up = is_tke ? interpf2c(upd_mean.values, grid, k, i) : upd_mean.values[i, k]
            GMVcov_ += up_prog[i].area[k] * (mean_up - GMVv_)^2
            Upd_cubed += up_prog[i].area[k] * mean_up^3
        end

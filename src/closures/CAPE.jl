...
"""
function compute_CAPE_old!(edmf::EDMFModel, state::State, param_set::APS, grid::Grid; terminate_on_neg_buoyancy::Bool=false)
    FT = float_type(state)
    ...
    @inbounds for k in real_center_indices(grid)
        ...
        if edmf.moisture_model isa EquilibriumMoisture
            ...
            ts_parcel = PhaseEquil_pθq_given_liquid_fraction(thermo_params, p_env, current_parcel_θ_liq, current_parcel_q_tot, liq_frac) # assume all liquid for CAPE calculation, since we do not use pow_icenuc anywhere else in NonEq, and the ice timescale is slower.
...
"""
function compute_CAPE!(edmf::EDMFModel, state::State, param_set::APS, grid::Grid; do_quadrature::Bool=false, terminate_on_neg_buoyancy::Bool=false)
    FT = float_type(state)
    thermo_params = TCP.thermodynamics_params(param_set)
    ...
    function run_cape_core!(::Val{N_QUAD}) where {N_QUAD}
        ...
        inv_pi = FT(1 / π)
        if N_QUAD == 1
            slot_ξ_q = SA.SVector{1, FT}(0); slot_ξ_h = SA.SVector{1, FT}(0); slot_w = SA.SVector{1, FT}(1)
        else
            n_raw, w_raw = FastGaussQuadrature.gausshermite(N_QUAD)
            nodes = SA.SVector{N_QUAD, FT}(n_raw); weights = SA.SVector{N_QUAD, FT}(w_raw)
            tmp_ξ_q = SA.MVector{NUM_SLOTS, FT}(undef); tmp_ξ_h = SA.MVector{NUM_SLOTS, FT}(undef); tmp_w = SA.MVector{NUM_SLOTS, FT}(undef)
            idx = 1
            for i in 1:N_QUAD, j in 1:N_QUAD
                tmp_ξ_q[idx] = nodes[i]; tmp_ξ_h[idx] = nodes[j]; tmp_w[idx] = (weights[i] * weights[j]) * inv_pi; idx += 1
            end
            slot_ξ_q = SA.SVector(tmp_ξ_q); slot_ξ_h = SA.SVector(tmp_ξ_h); slot_w = SA.SVector(tmp_w)
        end

        # 2. Allocations
        s_qt = SA.MVector{NUM_SLOTS, FT}(undef); s_θl = SA.MVector{NUM_SLOTS, FT}(undef)
        s_cape = SA.MVector{NUM_SLOTS, FT}(undef); s_mse = SA.MVector{NUM_SLOTS, FT}(undef); s_w = SA.MVector{NUM_SLOTS, FT}(undef)
        ...
        p_w = SA.MVector{POOL_SIZE, FT}(undef); indices = SA.MVector{POOL_SIZE, Int}(undef)
        ...
        # 3. Surface Init
        ts_src = ts_env[kc_surf]; qt_mean = TD.total_specific_humidity(thermo_params, ts_src)
        θl_mean = TD.liquid_ice_pottemp(thermo_params, ts_src); p_src = TD.air_pressure(thermo_params, ts_src)
        z_src = grid.zc.z[kc_surf]; sqrt2 = FT(sqrt(2))
        var_q = aux_en.QTvar[kc_surf]; var_h = aux_en.Hvar[kc_surf]
        σ_q = sqrt(max(var_q, 0)); σ_h = sqrt(max(var_h, 0))
        if NUM_SLOTS > 1 && σ_q > eps(FT); σ_q = min(σ_q, -qt_mean / (sqrt2 * slot_ξ_q[1])); end
        r = zero(FT); if σ_q > eps(FT) && σ_h > eps(FT); r = clamp(aux_en.HQTcov[kc_surf] / (σ_q * σ_h), -1, 1); end
        σ_cond = sqrt(max(1 - r^2, 0)) * σ_h

        for i in 1:NUM_SLOTS
            if NUM_SLOTS == 1
                 s_qt[i] = qt_mean; s_θl[i] = θl_mean
            else
                 s_qt[i] = max(qt_mean + sqrt2 * σ_q * slot_ξ_q[i], zero(FT))
                 s_θl[i] = θl_mean + sqrt2 * (σ_h * r * slot_ξ_q[i] + σ_cond * slot_ξ_h[i])
            end
            s_cape[i] = zero(FT); s_w[i] = slot_w[i]
        ...
        @inbounds for k in real_center_indices(grid)
            if k == kc_surf; continue; end
            ts_e = ts_env[k]; p_env = TD.air_pressure(thermo_params, ts_e)
            ρ_env = TD.air_density(thermo_params, ts_e); z_k = grid.zc.z[k]
            kf = CCO.PlusHalf(k.i); dz = grid.zf.z[kf+1] - grid.zf.z[kf]

            ts_prev = ts_env[k-1]; z_prev = grid.zc.z[k-1]
            ...
            # B. Rivals
            qt_env_l = TD.total_specific_humidity(thermo_params, ts_e); θl_env_l = TD.liquid_ice_pottemp(thermo_params, ts_e)
            l_σ_q = sqrt(max(aux_en.QTvar[k], 0)); l_σ_h = sqrt(max(aux_en.Hvar[k], 0))
            if NUM_SLOTS > 1 && l_σ_q > eps(FT); l_σ_q = min(l_σ_q, -qt_env_l / (sqrt2 * slot_ξ_q[1])); end
            l_r = zero(FT); if l_σ_q > eps(FT) && l_σ_h > eps(FT); l_r = clamp(aux_en.HQTcov[k] / (l_σ_q * l_σ_h), -1, 1); end
            l_σ_cond = sqrt(max(1 - l_r^2, 0)) * l_σ_h

            for i in 1:NUM_SLOTS
                idx = NUM_SLOTS + i
                if NUM_SLOTS == 1
                    r_qt = qt_env_l; r_θl = θl_env_l
                else
                ...
                p_qt[idx] = r_qt; p_θl[idx] = r_θl; p_w[idx] = slot_w[i]
                p_cape[idx] = zero(FT); p_is_rival[idx] = 1
            end
            ...
            # D. Conservation
            displaced_energy = zero(FT); winning_weight = zero(FT)
            for i in (NUM_SLOTS + 1):POOL_SIZE
                loser = indices[i]
                if p_cape[loser] > 0; displaced_energy += p_cape[loser] * p_w[loser]; end
            end
            for i in 1:NUM_SLOTS
                winner = indices[i]
                if p_mse[winner] > -FT(Inf); winning_weight += p_w[winner]; end
            end
            
            total_cape = zero(FT); total_weight = zero(FT)
            for i in 1:NUM_SLOTS
                winner = indices[i]
                s_qt[i] = p_qt[winner]; s_θl[i] = p_θl[winner]; s_w[i] = p_w[winner]
                s_mse[i] = p_mse[winner]
                if p_mse[winner] > -FT(Inf)
                    s_cape[i] = p_cape[winner]
                    if winning_weight > eps(FT); s_cape[i] += displaced_energy / winning_weight; end
    ...
    if do_quadrature; run_cape_core!(Val(2)); else; run_cape_core!(Val(1)); end

    # Legacy Updraft Block [Unchanged]
    N_up = n_updrafts(edmf)
    @inbounds for k in real_center_indices(grid); aux_bulk.CAPE[k] = zero(FT); end
    @inbounds for i in 1:N_up
        up_cape = zero(FT)
        @inbounds for k in real_center_indices(grid)
            a_up = aux_up[i].area[k]
            if a_up > FT(1e-5)
                ...
                kf = CCO.PlusHalf(k.i); dz = grid.zf.z[kf+1] - grid.zf.z[kf]
                up_cape += b_up * dz
                if up_cape < 0; up_cape = zero(FT); end
                aux_up[i].CAPE[k] = up_cape; aux_bulk.CAPE[k] += a_up * up_cape
            else
                aux_up[i].CAPE[k] = zero(FT); up_cape = zero(FT)
            end
        end
    end
    @inbounds for k in real_center_indices(grid)
        total_up_area = sum(aux_up[i].area[k] for i in 1:N_up)
        if total_up_area > FT(1e-5); aux_bulk.CAPE[k] /= total_up_area; else; aux_bulk.CAPE[k] = 0.0; end
    end
    return nothing
end

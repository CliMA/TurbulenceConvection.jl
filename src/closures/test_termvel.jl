...
microphys_params = CloudMicrophysics.Parameters.CloudMicrophysicsParameters{
    Float64,
    Thermodynamics.Parameters.ThermodynamicsParameters{Float64},
}(
    0.024,
    2.26e-5,
    1000.0,
    ...
    0.0,
    43.58563716234225,
    6.149368373672493e-7,
    8.210052874283406e-8,
    151437.07675216204,
    0.65,
    0.44,
    0.63,
...
v_ice = TC.my_terminal_velocity(
    param_set,
    CM.CommonTypes.IceType(),
    CM.CommonTypes.Chen2022Type(),
    ρ_air,
    FT(1e-7);
    D_transition = FT(625e-6),
    Nt = FT(1),
)
...
accr_li = TC.accretion_liq_ice(param_set, 1e-4, 1e-6, 1.0, 0.5, TC.CMT.Chen2022Type(), FT(100), FT(Inf), 1.0)
nav = TC.int_nav_dr(
    param_set,
    CM.CommonTypes.IceType(),
    CM.CommonTypes.Chen2022Type(),
...
plot!(
    q,
    scale_func(v_ice_dmax5),
    label = "Ice Dmax5 - truncate tiny N",
    color = :lime,
    linestyle = :dashdot,
    linewidth = 2,
)
...
plot!(
    [q[1], q[end]],
    scale_func(+10 .^ [linthreshy, linthreshy]),
    color = :gray,
    linestyle = :dash,
    linewidth = 1,
    dpi = 1200,
...
v_ice = test_my_terminal_velocity(
    param_set,
    CM.CommonTypes.IceType(),
    CM.CommonTypes.Chen2022Type(),
    ρ_air,
    FT(1e-7);
    D_transition = FT(625e-6 / 2),
    Nt = FT(0.1),
    Dmax = 625e-6 / 2,
)

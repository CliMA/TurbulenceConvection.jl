...
v_ice_dmax3 = ((q, N) -> TC.my_terminal_velocity(microphys_params, CM.CommonTypes.IceType(), CM.CommonTypes.Chen2022Type(), ρ_air, q; D_transition=D_trans, Dmax=FT(62.5e-6 * 2), Nt=FT(N))).(q, q./(4/3 * ρ_i *  π * r_is^3) )
v_ice_dmax4 = ((q, N) -> TC.my_terminal_velocity(microphys_params, CM.CommonTypes.IceType(), CM.CommonTypes.Chen2022Type(), ρ_air, q; D_transition=D_trans, Dmax=FT(62.5e-6 * 2), Nt=FT(N))).(q, q./(4/3 * ρ_i *  π * r_0^3) )

# make plot
plot(q, v_rain, label = "Rain", xlabel = "q [kg/kg]", ylabel = "v [m/s]", title = "Terminal velocity vs q", color = :blue, linewidth=2, dpi=600, xaxis=:log10, yaxis=:symlog)

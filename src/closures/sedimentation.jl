...
function calculate_sedimentation_sources(
    param_set::APS,
    q_type::Union{CMT.LiquidType, CMT.IceType},
    ...
    if !grid_mean
        ...
        w_sed_to_other = w_sed # rn, we're not actually addting background wind to w_sed, so this is fine we can reuse the same object...

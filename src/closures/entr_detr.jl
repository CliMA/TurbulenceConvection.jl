    ...
    @inbounds for i in 1:N_up
        ...
        @inbounds for k in real_center_indices(grid)
            ...
            if aux_up[i].area[k] > 0.0
                ...
                # update nondimensional entr/detr
                aux_up[i].ε_ml_nondim[k] = ε_nondim
                aux_up[i].δ_ml_nondim[k] = δ_nondim
            else
...
function compute_ml_entr_detr!(
    ...
    @inbounds for i in 1:N_up
        ...
        ε_nondim = parent(aux_up[i].ε_ml_nondim)
        δ_nondim = parent(aux_up[i].δ_ml_nondim)
        ...
        @. aux_up[i].entr_ml = ifelse(aux_up[i].area > 0, aux_up[i].entr_sc, 0)
        @. aux_up[i].detr_ml = ifelse(aux_up[i].area > 0, aux_up[i].detr_sc, 0)

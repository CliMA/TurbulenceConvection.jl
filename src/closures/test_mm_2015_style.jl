...
if reload_MM2015 ||
   !isdefined(Main, :morrison_milbrandt_2015_style) ||
   !isdefined(Main, :morrison_milbrandt_2015_style_exponential_part_only)
    if call_from_TC
...
dqvdt = 1.2898707234822461e-5;
dTdt = 0.01384903619503847;

area = 0.09999999999999998;
ρ = 1.1297764844315694;
p = 72678.5848761201;
T = 224.23736891366676;
w = -0.1271288806541458;
τ_liq = 8.70519108187029e-12;
τ_ice = 2.0788375361402434e-12;
δ_0 = -2.3831539958081173e-5;
δ_0i = 0.0;
q = Thermodynamics.PhasePartition{Float64}(0.00046794210973515136, 0.0, 0.0004296824485967858);
q_eq = Thermodynamics.PhasePartition{Float64}(0.00046794210973515136, 6.216785765933966e-5, 3.833631770125848e-5);
Δt = 9.999999999480808;
ts = Thermodynamics.PhaseNonEquil{Float64}(
    -35197.11200601141,
    1.1297764844315694,
    Thermodynamics.PhasePartition{Float64}(0.00046794210973515136, 0.0, 0.0004340611316873781),
);
dTdt = 0.00045330389447046477;
dqvdt = 8.245975226821409e-10;
q = Thermodynamics.PhasePartition{Float64}(0.001976663686494517, 0.0, 2.5519121725500658e-20);
T = 262.691521171828;
p = 84043.30717958807;
ρ = 1.1133964727328423;
w = -0.0005480114811913959;
relaxation_timescale = TurbulenceConvection.GeometricLiqExponentialTScalingAndGeometricIceRelaxationTimescale{Float64}(
    5048.808821939158,
    0.1821615879079399,
    7.334264855778188,
    4.0874493700288115,
    ...
    0.10566549047690477,
    -7.215527259640249,
    0.0030042933101672126,
    -0.8520013654401164,
    true,
    true,
    TurbulenceConvection.RelaxationTimescaleArgs{Float64}(
        1.0e-6,
        1.0e-6,
...
if test_loop
    ...
    p_c = NCDatasets.NCDataset(nc_file, "r") do data
        data.group["reference"]["p_c"][z_ind + 1]
    end
    ρ_c = NCDatasets.NCDataset(nc_file, "r") do data
        data.group["reference"]["ρ_c"][z_ind + 1]
    end

    get_mm_2015_style_verification_data_from_nc_file(
        t_ind::Int,
        env_or_updraft::Symbol;
        nc_file::String = nc_file,
    ...
    for (i, t_ind) in enumerate(t_inds)
        ...
        for updraft_or_env in filter(x -> x in [:updraft, :env], keys(S))
            ...
            (; T, w, area, q_tot, q_liq, q_ice, dTdt, dqvdt, τ_ice, τ_liq, Δt, q, ts, ρ, q_vap, q_vap_0, q_eq) =
                get_mm_2015_style_verification_data_from_nc_file(t_ind, updraft_or_env)
            ...
            S_ql, S_qi = morrison_milbrandt_2015_style_exponential_part_only(
                ...
                Δt * Δt_factor,
                ts;
                opts = MM2015Opts{FT}(use_fix = true),
            )
            ...
            t_hit_liq_sat_big = t_δ_hit_value(big(0.0), big(δ_0), A_c_big, τ_big)
    ...
    S[:mean] = OrderedCollections.OrderedDict(
        t_ind => OrderedCollections.OrderedDict(
            ...
            :S_ql =>
                sum(S[r][t_ind][:S_ql] * S[r][t_ind][:area] for r in keys(S) if r != :mean) /
                sum(S[r][t_ind][:area] for r in keys(S) if r != :mean),
            :S_qi =>
                sum(S[r][t_ind][:S_qi] * S[r][t_ind][:area] for r in keys(S) if r != :mean) /
                sum(S[r][t_ind][:area] for r in keys(S) if r != :mean),
...
for t_ind in t_inds
    for updraft_or_env in updraft_or_envs
        ...
        @showprogress dt = 1 desc = "Computing..." for (i, Δt) in enumerate(Δts)
            ...
            # (q_vap_0 .* FT(NaN), q_vap_0 * FT(NaN))
                morrison_milbrandt_2015_style_exponential_part_only(
                    ...
                    Δt,
                    ts;
                    opts = MM2015Opts{FT}(use_fix = use_fix),
                )
        ...
        if yscale === :symlog
            ...
            plot!(
                Δts,
                symlog((qls .+ qis) .* factors, linthresh),
                label = "sum",
                color = :red,
                yformatter = yformatter,
                linewidth = 1,
        ...
        factors = (plot_rates ? 1 : Δts)
        ymin_l =
            yscale === :symlog ? minimum(symlog(((qls .* factors)..., (qls_exp .* factors)...), linthresh)) :
            minimum(((qls .* factors)..., (qls_exp .* factors)...))
        ymin_i =
            yscale === :symlog ? minimum(symlog(((qis .* factors)..., (qis_exp .* factors)...), linthresh)) :
            minimum(((qis .* factors)..., (qis_exp .* factors)...))
        ymax_l =
            yscale === :symlog ? maximum(symlog(((qls .* factors)..., (qls_exp .* factors)...), linthresh)) :
            maximum(((qls .* factors)..., (qls_exp .* factors)...))
        ymax_i =
            yscale === :symlog ? maximum(symlog(((qis .* factors)..., (qis_exp .* factors)...), linthresh)) :
            maximum(((qis .* factors)..., (qis_exp .* factors)...))

...
reload_TC = false
if reload_TC || !isdefined(Main, :TurbulenceConvection) || !isdefined(Main, :param_set) || !isdefined(Main, :thermo_params) || (typeof(thermo_params).parameters[1] != FT)
...
S_ql = 1.5740282460754777e-6; S_qi = 8.837601096511839e-8;
area = 0.049966362669001334; ρ = 1.4607008779607433; p = 74651.29711651805; T = 178.1058319793084; w = 0.018706561325269898; τ_liq = 6.656910134239524e-3; τ_ice = 0.14858242869377136; q_vap = 5.095529671220612e-10; q = Thermodynamics.PhasePartition{Float64}(0.00020172170899797012, 5.7675685444915575e-9, 0.0002017154318764585); q_eq = Thermodynamics.PhasePartition{Float64}(0.00020172170899797012, 7.545234665120191e-8, 2.9644619882828738e-8); Δt = 0.007696511708622092; ts = Thermodynamics.PhaseNonEquil{Float64}(-68295.73889316698, 1.4607008779607433, Thermodynamics.PhasePartition{Float64}(0.00020172170899797012, 5.7675685444915575e-9, 0.0002017154318764585)); dqvdt = 1.2898707234822461e-6; dTdt = 0.01384903619503847
...
T = 273.1; ρ = 0.7; q_liq = 1.778279410038923e-5; q_ice = 0.0; τ_liq = 2.22507385850719e-309; τ_ice = 2.22507385850719e-309; Δt = 1.0e-50; sat = :liqsat; sat_factor = -0.1; w = -10.0; dqvdt = 0.0; dTdt = 0.0; ts = Thermodynamics.PhaseNonEquil{Float64}(16347.138620801497, 0.7, Thermodynamics.PhasePartition{Float64}(0.006919818397398777, 1.778279410038923e-5, 0.0)); p = 55095.54778139178; q_vap = 0.006902035603298388; q = Thermodynamics.PhasePartition{Float64}(0.006919818397398777, 1.778279410038923e-5, 0.0); q_eq = Thermodynamics.PhasePartition{Float64}(0.006919818397398777, 0.006902436691973973, 0.006898425805218117); area = 1.0
...
T = 273.2; ρ = 1.0; q_liq = 0.0; q_ice = 1.0e-10; τ_liq = 1.0e-50; τ_ice = 1.0e-10; Δt = 0.010000000000000002; sat = :liqsat; sat_factor = -0.1; w = -10.0; dqvdt = 0.0; dTdt = 0.0; ts = Thermodynamics.PhaseNonEquil{Float64}(11582.682072221132, 1.0, Thermodynamics.PhasePartition{Float64}(0.00486533099436541, 0.0, 1.0e-10)); p = 78641.0320095916; q_vap = 0.00486533089436541; q = Thermodynamics.PhasePartition{Float64}(0.00486533099436541, 0.0, 1.0e-10); q_eq = Thermodynamics.PhasePartition{Float64}(0.00486533099436541, 0.00486514234325963, 0.004867027854317428); area = 1.0

area = 1.0; ρ = 0.7; p = 55512.5292119373; T = 275.0; w = -10.0; τ_liq = 0.010000000000000002; τ_ice = 100.0; q_vap = 0.007875108050448738; q = Thermodynamics.PhasePartition{Float64}(0.007875108150448737, 0.0, 1.0e-10); q_eq = Thermodynamics.PhasePartition{Float64}(0.007875108150448737, 0.00786098494726133, 0.00800221597913541); Δt = 1.0; ts = Thermodynamics.PhaseNonEquil{Float64}(20031.323030729105, 0.7, Thermodynamics.PhasePartition{Float64}(0.007875108150448737, 0.0, 1.0e-10)); dqvdt = 0.0; dTdt = 0.0; dqsl_dT = 0.0005336975485260993; dqsi_dT = 0.0005432859978098328

area = 1.0; ρ = 1.3; p = 102161.92786926191; T = 273.2; w = -10.0; τ_liq = 2.22507385850719e-309; τ_ice = 2.22507385850719e-309; q_vap = 0.003742417187122792; q = Thermodynamics.PhasePartition{Float64}(0.0037601999812231814, 0.0, 1.778279410038923e-5); q_eq = Thermodynamics.PhasePartition{Float64}(0.0037601999812231814, 0.003742417187122792, 0.0037438675802441753); Δt = 1.0e-50; ts = Thermodynamics.PhaseNonEquil{Float64}(8910.102808405085, 1.3, Thermodynamics.PhasePartition{Float64}(0.0037601999812231814, 0.0, 1.778279410038923e-5)); dqvdt = 0.0; dTdt = 0.0; δ_ql = 3.969202437239124e252; δ_qi = -8.831768582223793e-7; δ_0 = 0.0; δ_0i = -1.4503931213831651e-6

area = 1.0; ρ = 0.7; p = 55118.30555399315; T = 273.2; w = -10.0; τ_liq = 1.0e-6; τ_ice = 100.0; q_vap = 0.006950203347513758; q = Thermodynamics.PhasePartition{Float64}(0.006950203347513758, 1.0e-200, 1.0e-200); q_eq = Thermodynamics.PhasePartition{Float64}(0.006950203347513758, 0.006950203347513758, 0.006952896934739183); Δt = 3.162277660168379e-8; ts = Thermodynamics.PhaseNonEquil{Float64}(16533.748957298405, 0.7, Thermodynamics.PhasePartition{Float64}(0.006950203347513758, 1.0e-200, 1.0e-200)); dqvdt = 0.0; dTdt = 0.0; dqsl_dT = 0.0004791120152533068; dqsi_dT = 0.00047929769759083086

area = 1.0; ρ = 0.7; p = 55512.054696767365; T = 275.0; w = -1.0; τ_liq = 2.22507385850719e-309; τ_ice = 0.010000000000000002; q_vap = 0.00786098353495101; q = Thermodynamics.PhasePartition{Float64}(0.00786098363495101, 0.0, 1.0e-10); q_eq = Thermodynamics.PhasePartition{Float64}(0.00786098363495101, 0.00786098494726133, 0.00800221597913541); Δt = 1.0e-5; ts = Thermodynamics.PhaseNonEquil{Float64}(19997.763441520052, 0.7, Thermodynamics.PhasePartition{Float64}(0.00786098363495101, 0.0, 1.0e-10)); dqvdt = 0.0; dTdt = 0.0; dqsl_dT = 0.0005336975485260993; dqsi_dT = 0.0005432859978098328
...
T = 273.2; ρ = 0.7; q_liq = 0.0; q_ice = 1.778279410038923e-5; τ_liq = 2.22507385850719e-309; τ_ice = 2.22507385850719e-309; Δt = 1.0e-50; sat = :liqsat; sat_factor = -0.1; w = 0.0; dqvdt = 0.0; dTdt = 0.0; ts = Thermodynamics.PhaseNonEquil{Float64}(16528.457262263426, 0.7, Thermodynamics.PhasePartition{Float64}(0.00696825550033669, 0.0, 1.778279410038923e-5)); p = 55117.33851141129; q_vap = 0.0069504727062363; q = Thermodynamics.PhasePartition{Float64}(0.00696825550033669, 0.0, 1.778279410038923e-5); q_eq = Thermodynamics.PhasePartition{Float64}(0.00696825550033669, 0.006950203347513758, 0.006952896934739183); area = 1.0

# -------------------------------------- #
q_liq = q.liq; q_ice = q.ice; q_vap_0 = q_vap
# ===================================================================== #
...
if yscale === :symlog
    ...
    #
    plot!(Δts, symlog(qls_exp .* Δts, linthresh), label = "S_ql_exp", color = :green, linestyle = :dash, yformatter = yformatter, linewidth = 2)
    plot!(Δts, symlog(qis_exp .* Δts, linthresh), label = "S_qi_exp", color = :blue, linestyle = :dash, yformatter = yformatter, linewidth = 2)
    plot!(Δts, symlog((qls_exp .+ qis_exp) .* Δts, linthresh), label = "sum_exp", color = :red, linestyle = :dash, yformatter = yformatter, linewidth = 1)
    #
    plot!(Δts, symlog(qls_standard .* Δts, linthresh), label = "S_ql_standard", color = :green, linestyle = :dot, yformatter = yformatter, linewidth = 2)
    plot!(Δts, symlog(qis_standard .* Δts, linthresh), label = "S_qi_standard", color = :blue, linestyle = :dot, yformatter = yformatter, linewidth = 2)
    plot!(Δts, symlog((qls_standard .+ qis_standard) .* Δts, linthresh), label = "sum_standard", color = :red, linestyle = :dot, yformatter = yformatter, linewidth = 1)
...
ymin_l = yscale === :symlog ? nanminimum(symlog(((qls .* Δts)..., (qls_exp .* Δts)..., (qls_standard .* Δts)...), linthresh)) : nanminimum(((qls .* Δts)..., (qls_exp .* Δts)..., (qls_standard .* Δts)...))
ymin_i = yscale === :symlog ? nanminimum(symlog(((qis .* Δts)..., (qis_exp .* Δts)..., (qis_standard .* Δts)...), linthresh)) : nanminimum(((qis .* Δts)..., (qis_exp .* Δts)..., (qis_standard .* Δts)...))
ymax_l = yscale === :symlog ? nanmaximum(symlog(((qls .* Δts)..., (qls_exp .* Δts)..., (qls_standard .* Δts)...), linthresh)) : nanmaximum(((qls .* Δts)..., (qls_exp .* Δts)..., (qls_standard .* Δts)...))
ymax_i = yscale === :symlog ? nanmaximum(symlog(((qis .* Δts)..., (qis_exp .* Δts)..., (qis_standard .* Δts)...), linthresh)) : nanmaximum(((qis .* Δts)..., (qis_exp .* Δts)..., (qis_standard .* Δts)...))

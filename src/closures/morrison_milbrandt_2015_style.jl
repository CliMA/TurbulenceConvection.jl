...
function get_params_and_go_to_mixing_ratio(
    param_set::APS,
    area::FT,
    ρ::FT,
    p::FT,
...
τ_func(τ_liq::FT, τ_ice::FT, L_i::FT, c_p::FT, dqsl_dT::FT, Γ_i::FT) where {FT} =
    1 / (1 / τ_liq + (1 + (L_i / c_p) * dqsl_dT) * ((1 / τ_ice) / (Γ_i))) # Eqn C2
...
function q_new(
    q_old::TD.PhasePartition,
    S_ql_mix_Δt::FT,
    S_qi_mix_Δt::FT,
    dqvdt_Δt::FT;
    error_on_q_neg::Bool = true,
) where {FT}
    ...
    if error_on_q_neg && (new_q_liq < 0 || new_q_ice < 0)
        error(
            "Negative q detected, q_old = $q_old, S_ql_mix_Δt = $S_ql_mix_Δt, S_qi_mix_Δt = $S_qi_mix_Δt, new_q_liq = $new_q_liq, new_q_ice = $new_q_ice",
        )
    end
...
function q_new(
    ...
    if error_on_q_neg && (new_q_liq < 0 || new_q_ice < 0)
        error(
            "Negative q detected, q_old = $q_old, S_ql_mix = $S_ql_mix, S_qi_mix = $S_qi_mix, Δt = $Δt, new_q_liq = $new_q_liq, new_q_ice = $new_q_ice",
        )
    end
...
function q_new(
    ...
    if error_on_q_neg && (new_q_liq < 0 || new_q_ice < 0)
        error(
            "Negative q detected, q_old = $q_old, q_liq_mix = $q_liq_mix, q_ice_mix = $q_ice_mix, S_ql_mix_Δt = $S_ql_mix_Δt, S_qi_mix_Δt = $S_qi_mix_Δt, new_q_liq = $new_q_liq, new_q_ice = $new_q_ice",
        )
    end
...
function q_new(
    ...
    if error_on_q_neg && (new_q_liq < 0 || new_q_ice < 0)
        error(
            "Negative q detected, q_old = $q_old, q_liq_mix = $q_liq_mix, q_ice_mix = $q_ice_mix, S_ql_mix = $S_ql_mix, S_qi_mix = $S_qi_mix, Δt = $Δt, new_q_liq = $new_q_liq, new_q_ice = $new_q_ice",
        )
    end
...
function morrison_milbrandt_2015_get_new_status_helper_helper(
    param_set::APS,
    p::FT,
    new_q::TD.PhasePartition,
    q_eq::TD.PhasePartition,
    ts::TD.ThermodynamicState,
    Δt::FT,
    w::FT,
...
function morrison_milbrandt_2015_get_new_status_helper(
    param_set::APS,
    p::FT,
    q::TD.PhasePartition,
    q_eq::TD.PhasePartition,
...
function morrison_milbrandt_2015_get_new_status_helper_full_source(
    ...
    w::FT;
    error_on_q_neg::Bool = true,
) where {FT} # This version should be more floating point stable -- whatever you calculate in mixing ratio you should get exactly out.
...
S_func_WBF(A_c::FT, τ::FT, τ_ice::FT, δ_0::FT, Δt::FT, Γ_i::FT, q_sl::FT, q_si::FT, CF_mp::FT = one(FT)) where {FT} =
    S_func_no_WBF(A_c, τ, τ_ice, δ_0, Δt, Γ_i) + ((q_sl - q_si) / (τ_ice * Γ_i)) * CF_mp #  # QICON Eqn C7 # if τ_ice is infinite, this works just fine
...
S_func_zero_Δt_safe(
    S_func::Union{typeof(S_func_indiv_no_WBF), typeof(S_ql_func_indiv), typeof(S_qi_func_indiv)},
    A_c::FT,
    τ::FT,
    δ_0::FT,
    Δt::FT,
    Γ::FT;
    small_func::Union{typeof(issmallt), typeof(iszero)} = issmallt,
) where {FT} = small_func(Δt) ? FT(0) : S_func(A_c, τ, δ_0, Δt, Γ)

S_func_zero_Δt_safe(
    S_func::Union{typeof(S_func_no_WBF), typeof(S_ql_func), typeof(S_qi_func_no_WBF)},
    A_c::FT,
    τ::FT,
    τ_c::FT,
    δ_0::FT,
    ...
    Γ::FT;
    small_func::Union{typeof(issmallt), typeof(iszero)} = issmallt,
) where {FT} = small_func(Δt) ? FT(0) : S_func(A_c, τ, τ_c, δ_0, Δt, Γ)

S_func_zero_Δt_safe(
    S_func::Union{typeof(S_func_WBF), typeof(S_qi_func)},
    A_c::FT,
    τ::FT,
    τ_c::FT,
    δ_0::FT,
    ...
    q_si::FT;
    small_func::Union{typeof(issmallt), typeof(iszero)} = issmallt,
) where {FT} = small_func(Δt) ? FT(0) : S_func(A_c, τ, τ_c, δ_0, Δt, Γ, q_sl, q_si)
...
function get_t_out_of_q_no_WBF(δ_0::FT, A_c::FT, τ::FT, τ_c::FT, q_c::FT, Γ::FT, exit_if_fail::Bool = true) where {FT}
    ...
    if !iszero(A_c) # required for both
        if (δ_0 ≠ (A_c * τ)) # sol'n 1 is valid [almost a surety]
            ...
            if (
                lambert_argument =
                    exp(-(c + A_c * τ - δ_0) / (A_c * τ) + log(abs((δ_0 - A_c * τ) / (A_c * τ)))) *
                    sign(δ_0 - A_c * τ) *
                    sign(A_c * τ)
            ) > -exp(-1) # this is more accurate...
                ...
                if exit_if_fail && (iszero(lambert_argument) || isinf(lambert_argument)) && !iszero(q_c)
                    ...
                    for LW_branch in ((lambert_argument < FT(0)) ? (0, -1) : (0,))
                        t_candidate =
                            (A_c * τ * LambertW.lambertw(lambert_argument, LW_branch) + A_c * τ + c - δ_0) / A_c # for some reason this seemed more stable in paractice....
...
function get_t_out_of_q_WBF(
    ...
    if !iszero(A_c + B)
        if (δ_0 ≠ (A_c * τ)) # sol'n 1 is valid [almost a surety]
            ...
            if (
                lambert_argument = exp(
                    -(c + A_c * τ - δ_0) / ((A_c + B) * τ) +
                    log(abs((δ_0 - A_c * τ) / ((A_c + B) * τ))) * sign(δ_0 - A_c * τ) * sign((A_c + B) * τ),
                )
            ) > -exp(-1) # this is more accurate...
                if exit_if_fail && iszero(lambert_argument) && !iszero(q_ice)
                    ...
                    for LW_branch in ((lambert_argument < FT(0)) ? (0, -1) : (0,))
                        t_candidate =
                            (τ * (A_c + B) * LambertW.lambertw(lambert_argument, LW_branch) + A_c * τ + c - δ_0) /
                            (A_c + B) # # for some reason this seemed more stable in paractice....
        ...
        if !iszero(A_c) && !iszero(A_c + B) && !iszero(c)
            if (lambert_argument = -A_c * exp(-(c + A_c * τ) / ((A_c + B) * τ)) / (A_c + B)) > -exp(-1) # within lambertw domain
                if exit_if_fail && iszero(lambert_argument) && !iszero(q_ice)
                    ...
                    for LW_branch in ((lambert_argument < FT(0)) ? (0, -1) : (0,))
                        t_candidate =
                            (τ * (A_c + B) * LambertW.lambertw(lambert_argument, LW_branch) + A_c * τ + c) / (A_c + B)
                        if t_candidate > FT(0)
...
function get_t_T_hit_T_freeze(
    ...
    if !iszero(A_c * K + B)
        if (δ_0 ≠ (A_c * τ)) # sol'n 1 is valid [almost a surety]
            if (
                lambert_argument =
                    exp(-(c + A_c * K * τ - K * δ_0) / ((A_c * K + B) * τ)) * K * (δ_0 - A_c * τ) / ((A_c * K + B) * τ)
            ) > -exp(-1) # within lambertw domain
                if exit_if_fail && iszero(lambert_argument) && !iszero(c)
                    ...
                    for LW_branch in ((lambert_argument < FT(0)) ? (0, -1) : (0,))
                        t_candidate =
                            (
                                τ * (A_c * K + B) * LambertW.lambertw(lambert_argument, LW_branch) + A_c * K * τ + c -
                                K * δ_0
                            ) / (A_c * K + B)
...
target(value::FT, add_or_sub::Union{typeof(+), typeof(-)}; factor::FT = 1.0) where {FT} =
    add_or_sub(value, residual_tol(value, factor = factor))
...
function get_t_var_hit_value(
    var::Union{Val{:δ}, Val{:δi}, Val{:δip}, Val{:δipl}, Val{:T}},
    param_set::APS,
    value::FT,
    min_t::FT,
    max_t::FT,
    ...
    if min_t > FT(0)
        var_at_min_t = get_t_var_hit_value_helper_Δt_safe(
            var,
            param_set,
            δ_0,
            A_c,
    ...
    var_at_max_t = get_t_var_hit_value_helper_Δt_safe(
        var,
        param_set,
        δ_0,
        A_c,
    ...
    if signbit(var_at_max_t - value) == signbit(var_at_min_t - value) # there's no root in the interval, could also use and 
        ...
        while num_check_bisect_endpoints > 0
            ...
            new_max_t /= FT(2)
            var_at_new_max_t = get_t_var_hit_value_helper_Δt_safe(
                var,
                param_set,
                δ_0,
                A_c,
    ...
    if check_predicted_t # we calculat these outside anyway, so we should probably turn this off normally
        ...
        if t_guess < max_t
            var_at_t_guess = get_t_var_hit_value_helper_Δt_safe(
                var,
                param_set,
                δ_0,
                A_c,
                ...
                error_on_q_neg = false,
                use_WBF = use_WBF,
            ) # this should be an exact function
    ...
    val_min_t =
        get_t_var_hit_value_helper_Δt_safe(
            var,
            param_set,
            δ_0,
            A_c,
...
function get_t_var_hit_value_helper(
    ::Val{:δ},
    param_set::APS,
    δ_0::FT,
    A_c::FT,
    ...
    S_qi =
        use_WBF ? S_qi_func(A_c, τ, τ_ice, δ_0, Δt, Γ_i, q_sl, q_si) :
        S_qi_func_no_WBF(A_c, τ, τ_ice, δ_0 + (q_sl - q_si), Δt, Γ_i)

    (; new_q_vap, new_q_sl) = morrison_milbrandt_2015_get_new_status_helper(
        param_set,
        p,
        q,
        q_eq,
        ...
        ts,
        w;
        error_on_q_neg = error_on_q_neg,
    )

    return new_q_vap - new_q_sl
end

function get_t_var_hit_value_helper_return_outputs(
    ::Val{:δ},
    param_set::APS,
    δ_0::FT,
    A_c::FT,
    ...
    S_qi =
        use_WBF ? S_qi_func(A_c, τ, τ_ice, δ_0, Δt, Γ_i, q_sl, q_si) :
        S_qi_func_no_WBF(A_c, τ, τ_ice, δ_0 + (q_sl - q_si), Δt, Γ_i)
    (; new_q_vap, new_q_sl, new_T) = morrison_milbrandt_2015_get_new_status_helper(
        param_set,
        p,
        q,
        q_eq,
...
function get_t_var_hit_value_helper(
    ...
    S_ql =
        use_WBF ? S_func_WBF(A_c, τ, τ_liq, δ_0, Δt, Γ_l, q_si, q_sl, CF_mp) :
        S_func_no_WBF(A_c, τ, τ_liq, δ_0 - (q_si - q_sl), Δt, Γ_l)
    (; new_q_vap, new_q_si) = morrison_milbrandt_2015_get_new_status_helper(
        param_set,
        p,
        q,
        q_eq,
...
function get_t_var_hit_value_helper_return_outputs(
    ...
    S_ql =
        use_WBF ? S_func_WBF(A_c, τ, τ_liq, δ_0, Δt, Γ_l, q_si, q_sl, CF_mp) :
        S_func_no_WBF(A_c, τ, τ_liq, δ_0 - (q_si - q_sl), Δt, Γ_l)
    (; new_q_vap, new_q_si, new_T) = morrison_milbrandt_2015_get_new_status_helper(
        param_set,
        p,
        q,
        q_eq,
...
function get_t_var_hit_value_helper(
    ...
    S_ql =
        use_WBF ? S_func_WBF(A_c, τ, τ_liq, δ_0, Δt, Γ_l, q_si, q_sl, CF_mp) :
        S_func_no_WBF(A_c, τ, τ_liq, δ_0 - (q_si - q_sl), Δt, Γ_l) # swap l for i
    S_qi = S_func_no_WBF(A_c, τ, τ_ice, δ_0, Δt, Γ_i)
    (; new_q_vap, new_q_sl) = morrison_milbrandt_2015_get_new_status_helper(
        param_set,
        p,
...
function get_t_var_hit_value_helper(
    ...
    S_qi =
        use_WBF ? S_qi_func(A_c, τ, τ_ice, δ_0, Δt, Γ_i, q_sl, q_si) :
        S_qi_func_no_WBF(A_c, τ, τ_ice, δ_0 + (q_sl - q_si), Δt, Γ_i)
    (; new_T) = morrison_milbrandt_2015_get_new_status_helper(
        param_set,
        p,
        q,
        q_eq,
...
# --- #
function get_t_var_hit_value_helper_Δt_safe(
    var::Val{:δ},
    param_set::APS,
    δ_0::FT,
    A_c::FT,
    ...
    error_on_q_neg::Bool = true,
    small_func::Union{typeof(issmallt), typeof(iszero)} = issmallt,
    large_func::typeof(limit_large_Δt) = limit_large_Δt,
    use_WBF::Bool = true,
) where {FT}
    if small_func(Δt)
        return (q_vap - q_sl)
        ...
        Δt = large_func(Δt)
        return get_t_var_hit_value_helper(
            var,
            param_set,
            δ_0,
            A_c,
...
function get_t_var_hit_value_helper_return_outputs_Δt_safe(
    var::Val{:δ},
    param_set::APS,
    δ_0::FT,
    A_c::FT,
    ...
    if small_func(Δt)
        ...
        Δt = large_func(Δt)
        return get_t_var_hit_value_helper_return_outputs(
            var,
            param_set,
            δ_0,
            A_c,
...
function get_t_var_hit_value_helper_return_outputs_Δt_safe(
    ...
    if small_func(Δt)
        ...
        Δt = large_func(Δt)
        get_t_var_hit_value_helper_return_outputs(
            var,
            param_set,
            δ_0,
            A_c,
...
function morrison_milbrandt_2015_style(
    ...
    if area > 0
        ...
        return morrison_milbrandt_2015_style(
            regime,
            param_set,
            area,
            ρ,
...
# ====================================================================================================================================================================================================== #
# ====================================================================================================================================================================================================== #
@inline min_if_converged(x::FT, x_candidate::FT, x_candidate_converged::Bool) where {FT} =
    x_candidate_converged ? min(x, x_candidate) : x # helper function to update the shortest time only if converged
...
@inline S_above_floor(S_ql::FT, S_qi::FT, q_liq::FT, q_ice::FT, Δt::FT) where {FT} =
    S_above_floor(S_ql, q_liq, Δt), S_above_floor(S_qi, q_ice, Δt) # ensure S_ql, S_qi do not deplete q_liq, q_ice in floating point
"""
...
function morrison_milbrandt_2015_style(
    regime::Union{
        Supersaturated{true, true, true},
        Supersaturated{true, false, true},
        Supersaturated{false, true, true},
        Supersaturated{false, false, true},
    }, # these should all work the same, right? you'll end up with some liq/ice at the end no matter what
    param_set::APS,
    area::FT,
    ρ::FT,
    p::FT,
    ...
    if depth == max_depth
        ...
        return return_mixing_ratio ? (S_shum_to_mixing_ratio(S_ql, q.tot), S_shum_to_mixing_ratio(S_qi, q.tot)) :
               (S_ql, S_qi) # exponential only doesn't bother with mixing ratios at the moment
    end
    ...
    # --- Thermo  constants ------------------------------------------------------------------------------------ #
    (; g, L_i, L_l, c_p, e_sl, e_si, dqsl_dT, dqsi_dT, q_sl, q_si, q_vap, q_liq, q_ice, T_freeze, δ_0, δ_0i, Γ_l, Γ_i) =
        get_params_and_go_to_mixing_ratio(
            param_set,
            area,
            ρ,
            p,
    ...
    if !t_out_of_liq_valid
        if fallback_to_standard_supersaturation_limiter
            ...
            return standard_supersaturation_sources(
                ...
                Δt,
                ts,
            ) # use this version with δ_0, δ_0i to keep our up to date supersaturation
        end
    ...
    t_hit_δ_sat, t_hit_δ_sat_converged = get_t_var_hit_value(
        Val{:δ}(),
        param_set,
        target(FT(0), -; factor = FT(TF)),
        FT(0),
        max_t,
        δ_0,
        A_c,
        ...
        q_ice,
        ts,
    ) # if didn't hit sat and t_hit_sat is the least, it'll just equal Δt, then we can use these S_ql and S_qi and shouldn't have to recalculate them at the end...
    ...
    max_t = min_if_converged(max_t, t_hit_δ_sat, t_hit_δ_sat_converged)
    t_hit_T_freeze, t_hit_T_freeze_converged = get_t_var_hit_value(
        Val{:T}(),
        param_set,
        target(T_freeze, +; factor = FT(TF)),
        FT(0),
        max_t,
        δ_0,
        A_c,
    ...
    if (min_t < Δt)
        if i_min_t == 1 # out of liquid
            ...
            new_ρ, new_p, new_T, new_q_vap, new_q, new_q_eq, new_q_sl, new_q_si, new_ts =
                morrison_milbrandt_2015_get_new_status_helper_full_source(
                    param_set,
                    p,
                    q,
                    q_eq,
            ...
            S_ql_addit, S_qi_addit = morrison_milbrandt_2015_style(
                ...
                opts = MM2015Opts{FT}(
                    ...
                    depth = depth + 1,
                    δ_0_shum = new_δ_0_shum,
                    δ_0i_shum = new_δ_0i_shum,
                    dqvdt = dqvdt,
                    dTdt = dTdt,
                    fallback_to_standard_supersaturation_limiter = fallback_to_standard_supersaturation_limiter,
                ),
            ...
            return return_mixing_ratio ? (S_ql + S_ql_addit, S_qi + S_qi_addit) :
                   (S_mixing_ratio_to_shum(S_ql + S_ql_addit, q.tot), S_mixing_ratio_to_shum(S_qi + S_qi_addit, q.tot))
        ...
        elseif i_min_t == 2 # out of ice
            ...
            S_ql, S_qi = S_above_floor(S_ql, S_qi, q_liq, q_ice, min_t) # if t was too small for t_out_of_q for example, this makes sure we put a floor on S_ql and S_qi
            new_ρ, new_p, new_T, new_q_vap, new_q, new_q_eq, new_q_sl, new_q_si, new_ts =
                morrison_milbrandt_2015_get_new_status_helper(
                    param_set,
                    p,
                    q,
                    q_eq,
        ...
        return return_mixing_ratio ? (S_ql, S_qi) :
               (S_mixing_ratio_to_shum(S_ql, q.tot), S_mixing_ratio_to_shum(S_qi, q.tot))
...
function morrison_milbrandt_2015_style(
    regime::Union{
        Supersaturated{true, true, false},
        Supersaturated{true, false, false},
        Supersaturated{false, true, false},
        Supersaturated{false, false, false},
    },# these should all work the same, right? you'll end up with some liq/ice at the end no matter what
    param_set::APS,
    area::FT,
    ρ::FT,
...
function morrison_milbrandt_2015_style(
    ...
    max_t = Δt
    (t_out_of_liq, t_out_of_liq_valid) =
        use_ice_perspective ? get_t_out_of_q_WBF(δ_0, A_c, τ, τ_liq, q_liq, Γ_l, q_si, q_sl, true, CF_mp) :
        get_t_out_of_q_liq(δ_0, A_c, τ, τ_liq, q_liq, Γ_l) # don't calculate if we're starting with no liquid, calculation would fail
    (t_out_of_ice, t_out_of_ice_valid) =
        use_ice_perspective ? get_t_out_of_q_ice_no_WBF(δ_0, A_c, τ, τ_ice, q_ice, Γ_i) :
        get_t_out_of_q_ice(δ_0, A_c, τ, τ_ice, q_ice, Γ_i, q_sl, q_si) # don't calculate if we're starting with no ice, calculation would fail
    ...
    if !t_out_of_liq_valid
        ...
        (t_out_of_liq, t_out_of_liq_vaid) =
            use_ice_perspective ?
            FT(
                get_t_out_of_q_WBF(
                    big(δ_0),
            ...
            ) : FT(get_t_out_of_q_liq(big(δ_0), A_c_big, τ_big, big(τ_liq), big(q_liq), big(Γ_l), false).sol) # don't exit again if fail...
    ...
    if !t_out_of_ice_valid
        ...
        if !(@isdefined A_c_big)
            ...
            τI_big =
                (@isdefined τI_big) ? τI_big :
                τ_func(big(τ_ice), big(τ_liq), big(L_l), big(c_p), big(dqsi_dT), big(Γ_l))
        ...
        t_out_of_ice =
            use_ice_perspective ?
            FT(get_t_out_of_q_ice_no_WBF(big(δ_0), A_c_big, τ_big, big(τ_ice), big(q_ice), big(Γ_i), false).sol) :
            FT(
                get_t_out_of_q_ice(
                    big(δ_0),
                    A_c_big,
    ...
    max_t = min(max_t, t_out_of_liq, t_out_of_ice)
    t_hit_δ_sat, t_hit_δ_sat_converged = get_t_var_hit_value(
        use_ice_perspective ? Val{:δip}() : Val{:δ}(),
        param_set,
        target(FT(0), use_ice_perspective ? (-) : (+); factor = FT(TF)),
        FT(0),
        max_t,
        δ_0,
        A_c,
    ...
    max_t = min_if_converged(max_t, t_hit_δ_sat, t_hit_δ_sat_converged)
    t_hit_δo_sat, t_hit_δo_sat_converged = get_t_var_hit_value(
        use_ice_perspective ? Val{:δipl}() : Val{:δ}(),
        param_set,
        target(FT(0), use_ice_perspective ? (+) : (-); factor = FT(TF)),
        FT(0),
        max_t,
    ...
    if (min_t < Δt)
        if i_min_t == 1 # out of liquid
            S_ql = -q_liq / min_t
            S_qi =
                use_ice_perspective ? S_func_no_WBF(A_c, τ, τ_ice, δ_0, min_t, Γ_i) :
                S_qi_func(A_c, τ, τ_ice, δ_0, min_t, Γ_i, q_sl, q_si)
            ...
            new_ρ, new_p, new_T, new_q_vap, new_q, new_q_eq, new_q_sl, new_q_si, new_ts =
                morrison_milbrandt_2015_get_new_status_helper_full_source(
                    ...
                    w;
                    error_on_q_neg = false,
                ) # use multiplied form for floating point accuracy
            Δt_left = Δt - min_t
            new_regime = get_saturation_regime(new_q_vap, new_q.liq, new_q.ice, new_q_sl, new_q_si, T, T_freeze)
            # @debug "Ran out of liquid before timestep is over... transitioning to $(typeof(new_regime)) at t = $(min_t)..."
            new_δ_0_shum = TD.vapor_specific_humidity(new_q) - new_q_eq.liq
        ...
        elseif i_min_t == 2  # out of ice
            # S_ql = S_ql_func( A_c, τ, τ_liq, δ_0, min_t, Γ_l)
            S_ql =
                use_ice_perspective ? S_func_WBF(A_c, τ, τ_liq, δ_0, min_t, Γ_l, q_si, q_sl, CF_mp) :
                S_ql_func(A_c, τ, τ_liq, δ_0, min_t, Γ_l) # however much happens in that time, rescaled to the timestep
            S_qi = -q_ice / min_t
        ...
        # @debug "nothing of note through end of timestep..."
        S_ql =
            use_ice_perspective ? S_func_WBF(A_c, τ, τ_liq, δ_0, Δt, Γ_l, q_si, q_sl) :
            S_ql_func(A_c, τ, τ_liq, δ_0, Δt, Γ_l) # however much happens in that time, rescaled to the timestep
        S_qi =
            use_ice_perspective ? S_func_no_WBF(A_c, τ, τ_ice, δ_0, Δt, Γ_i) :
            S_qi_func(A_c, τ, τ_ice, δ_0, Δt, Γ_i, q_sl, q_si)
...
function morrison_milbrandt_2015_style(
    ...
    if !t_out_of_liq_valid
        ...
        t_out_of_liq =
            ...
            ) : FT(get_t_out_of_q_liq(δ_0_big, A_c_big, τ_big, big(τ_liq), big(q_liq), big(Γ_l), false).sol) # don't calculate if we're starting with no liquid, calculation would fail
        # @debug "After upgrading to BigFloat, t_out_of_liq = $t_out_of_liq"
    end
    ...
    max_t = min_if_converged(max_t, t_hit_δ_sat, t_hit_δ_sat_converged)
    t_hit_δo_sat, t_hit_δo_sat_converged = get_t_var_hit_value(
        use_ice_perspective ? Val{:δipl}() : Val{:δi}(),
        param_set,
        target(FT(0), use_ice_perspective ? (-) : (+); factor = FT(TF)),
        FT(0),
        max_t,
    ...
    if (min_t < Δt)
        if i_min_t == 1 # out of liq, we're done
            ...
            if (S_qi > FT(0))
                ...
                ) # we had ice growth instead of loss, set τ_ice to infinity and recall
            end
        ...
        # @debug "nothing of note through end of timestep..."
        S_ql =
            use_ice_perspective ? S_func_WBF(A_c, τ, τ_liq, δ_0, min_t, Γ_l, q_si, q_sl) :
            S_ql_func(A_c, τ, τ_liq, δ_0, min_t, Γ_l) # however much happens in that time, rescaled to the timestep
        S_qi = FT(0)
...
function morrison_milbrandt_2015_style(
    ...
    if !t_out_of_liq_valid
        ...
        t_out_of_liq =
            ...
            ) : FT(get_t_out_of_q_liq(δ_0_big, A_c_big, τ_big, big(τ_liq), big(q_liq), big(Γ_l), false).sol)
        # @debug "After upgrading to BigFloat, t_out_of_liq = $t_out_of_liq"
    end
    if !t_out_of_ice_valid
        if fallback_to_standard_supersaturation_limiter
        ...
        t_out_of_ice =
            use_ice_perspective ?
            FT(get_t_out_of_q_ice_no_WBF(δ_0_big, A_c_big, τ_big, big(τ_ice), big(q_ice), big(Γ_i), false).sol) :
            FT(
                get_t_out_of_q_ice(
                    δ_0_big,
                    A_c_big,
...
function morrison_milbrandt_2015_style(
    ...
    (t_out_of_ice, t_out_of_ice_valid) =
        use_ice_perspective ? get_t_out_of_q_ice_no_WBF(δ_0, A_c, τ, τ_ice, q_ice, Γ_i) :
        get_t_out_of_q_ice(δ_0, A_c, τ, τ_ice, q_ice, Γ_i, q_sl, q_si) # no liq growth while subsat, only ice changes
    ...
    if (min_t < Δt)
        if i_min_t == 1 # out of ice
            ...
            S_ql = FT(0) # no liq gain during WBF and started with none
            S_qi =
                use_ice_perspective ? S_func_indiv_no_WBF(A_c, τ, δ_0, min_t, Γ_i) :
                S_func_indiv_no_WBF(A_c, τ, δ_0i, min_t, Γ_i)
        ...
        S_ql = FT(0)
        S_qi =
            use_ice_perspective ? S_func_indiv_no_WBF(A_c, τ, δ_0, Δt, Γ_i) : S_func_indiv_no_WBF(A_c, τ, δ_0i, Δt, Γ_i)
        S_ql, S_qi = S_above_floor(S_ql, S_qi, q_liq, q_ice, Δt) # if t was too small for t_out_of_q for example, this makes sure we put a floor on S_ql and S_qi
        return return_mixing_ratio ? (S_ql, S_qi) :
               (S_mixing_ratio_to_shum(S_ql, q.tot), S_mixing_ratio_to_shum(S_qi, q.tot))
    end
...
function morrison_milbrandt_2015_style(
    ...
    t_hit_δi_sat, t_hit_δi_sat_converged = get_t_var_hit_value(
        Val{:δi}(),
        param_set,
        target(FT(0), +; factor = FT(TF)),
        FT(0),
...
function morrison_milbrandt_2015_style(
    ...
    if !t_out_of_ice_valid
        ...
        t_out_of_ice =
            ...
            ) # don't calculate if we're starting with no ice, calculation would fail
    ...
    if BF # below freezing, heading for ice sat first, from ice perspective (δip) or from liq perspective (δi)
        t_hit_sat, t_hit_sat_converged = get_t_var_hit_value(
            use_ice_perspective ? Val{:δip}() : Val{:δi}(),
            param_set,
            target(FT(0), +; factor = FT(TF)),
            FT(0),
            max_t,
        ...
        t_hit_sat, t_hit_sat_converged = get_t_var_hit_value(
            ...
            q_ice,
            ts,
        ) # if didn't hit sat and t_hit_sat is the least, it'll just equal Δt, then we can use these S_ql and S_qi and shouldn't have to recalculate them at the
    end
    max_t = min_if_converged(max_t, t_hit_sat, t_hit_sat_converged)
    t_hit_T_freeze, t_hit_T_freeze_converged = get_t_var_hit_value(
        Val{:T}(),
        param_set,
        target(T_freeze, BF ? (+) : (-); factor = FT(TF)),
        FT(0),
        max_t,
        δ_0,
        A_c,
    ...
    if (min_t < Δt)
        ...
        elseif i_min_t == 2 # out of ice
            S_ql =
                use_ice_perspective ? S_func_WBF(A_c, τ, τ_liq, δ_0, min_t, Γ_l, q_si, q_sl, CF_mp) :
                S_ql_func(A_c, τ, τ_liq, δ_0, min_t, Γ_l)
            S_qi = -q_ice / min_t
        ...
        S_ql =
            use_ice_perspective ? S_func_WBF(A_c, τ, τ_liq, δ_0, Δt, Γ_l, q_si, q_sl) :
            S_ql_func(A_c, τ, τ_liq, δ_0, Δt, Γ_l)
        S_qi =
            use_ice_perspective ? S_func_no_WBF(A_c, τ, τ_ice, δ_0, Δt, Γ_i) :
            S_qi_func(A_c, τ, τ_ice, δ_0, Δt, Γ_i, q_sl, q_si)
...
function morrison_milbrandt_2015_style(
    ...
    (q_vap < q_si) ||
        error("should be subsaturated, got q_vap = $q_vap, q_si = $q_si, (q_vap - q_si) = $(q_vap - q_si)")
    ...
    if !t_out_of_ice_valid
        ...
        t_out_of_ice =
            FT(get_t_out_of_q_ice_no_WBF(big(δ_0i), A_c_big, big(τ), big(τ_ice), big(q_ice), big(Γ_i), false).sol) # calcualte as if is liquid since  there's no WBF
        # @debug "After upgrading to big, t_out_of_ice = $t_out_of_ice"
    end

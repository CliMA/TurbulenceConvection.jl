...
function hydrometeor_covariances(
    ...
    sigmas = StaticArrays.SVector{6, FT}(
        _sigma(var_ql, mean_ql),
        _sigma(var_qr, mean_qr),
        _sigma(var_qi, mean_qi),
        _sigma(var_qs, mean_qs),
        _sigma(var_qt, mean_qt),
        _sigma(var_h, mean_h),
    )
    ...
    std_over_mean = StaticArrays.SVector{6, FT}(
        _norm(sigmas[1], mean_ql),
        _norm(sigmas[2], mean_qr),
        _norm(sigmas[3], mean_qi),
        _norm(sigmas[4], mean_qs),
        _norm(sigmas[5], mean_qt),
        _norm(sigmas[6], mean_h),
    )
    ...
    rhos_w = StaticArrays.SVector{6, FT}(
        _rho(flux_ql, sigmas[1], sigma_w),
        _rho(flux_qr, sigmas[2], sigma_w),
        _rho(flux_qi, sigmas[3], sigma_w),
        _rho(flux_qs, sigmas[4], sigma_w),
        _rho(flux_qt, sigmas[5], sigma_w),
        _rho(flux_h, sigmas[6], sigma_w),
    )
...
function hydrometeor_correlations_from_covariance(Cov::StaticArrays.SMatrix{7, 7, FT}) where {FT}
    ...
    return (
        corr_w_ql = _corr(Cov, 1, 2),
        corr_w_qr = _corr(Cov, 1, 3),
        corr_w_qi = _corr(Cov, 1, 4),
        corr_w_qs = _corr(Cov, 1, 5),
        corr_w_qt = _corr(Cov, 1, 6),
        corr_w_h = _corr(Cov, 1, 7),
        corr_ql_qr = _corr(Cov, 2, 3),
        corr_ql_qi = _corr(Cov, 2, 4),
        corr_ql_qs = _corr(Cov, 2, 5),
        corr_ql_qt = _corr(Cov, 2, 6),
        corr_ql_h = _corr(Cov, 2, 7),
        corr_qr_qi = _corr(Cov, 3, 4),
        corr_qr_qs = _corr(Cov, 3, 5),
        corr_qr_qt = _corr(Cov, 3, 6),
        corr_qr_h = _corr(Cov, 3, 7),
        corr_qi_qs = _corr(Cov, 4, 5),
        corr_qi_qt = _corr(Cov, 4, 6),
        corr_qi_h = _corr(Cov, 4, 7),
        corr_qs_qt = _corr(Cov, 5, 6),
        corr_qs_h = _corr(Cov, 5, 7),
...
"""
function hydrometeor_correlations(
    ::Type{FT};
    rho_w_ql = FT(0),
    rho_w_qr = FT(0),
    rho_w_qi = FT(0),
    ...
    rho_w_qt = FT(0),
    rho_w_h = FT(0),
    std_over_mean_ql = FT(1),
    std_over_mean_qr = FT(1),
    std_over_mean_qi = FT(1),
    std_over_mean_qs = FT(1),
    std_over_mean_qt = FT(1),
    std_over_mean_h = FT(1),
    alpha = FT(0.19),
) where {FT <: AbstractFloat}

    rhos = StaticArrays.SVector{6, FT}(rho_w_ql, rho_w_qr, rho_w_qi, rho_w_qs, rho_w_qt, rho_w_h)
...
@inline function _csigma_core(
    ...
    @inbounds for i in 2:last(rows)
        for j in (i + 1):last(rows)
            f_ij = clamp(
                alpha * std_over_mean[i - 1] * std_over_mean[j - 1] * sign(c_param[1, i] * c_param[1, j]),
                T(-0.50),
                T(0.50),
            )
    ...
    result = StaticArrays.SMatrix{Dim, Dim, T}((i == j ? one(T) : Sigma[i, j]) for i in 1:Dim, j in 1:Dim)

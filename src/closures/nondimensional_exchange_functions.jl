...
function non_dimensional_groups(param_set, εδ_model_vars)
    ...
    eps_FT = eltype(εδ_model_vars.tke)
    Π_1 = Δw^2 / (εδ_model_vars.zc_i * Δb)
    Π_2 = Δw^2 / (εδ_model_vars.tke + eps(eps_FT))
...
function non_dimensional_function(param_set, εδ_model_vars, ::NNEntr)
    ...
    nn_arc = (4, 4, 3, 2)  # (#inputs, #neurons, #outputs)
    nn_model = Flux.Chain(
        Flux.Dense(reshape(c_gen[1:16], nn_arc[2], nn_arc[1]), c_gen[17:20], Flux.sigmoid),
        Flux.Dense(reshape(c_gen[21:32], nn_arc[3], nn_arc[2]), c_gen[33:35], Flux.sigmoid),
        Flux.Dense(reshape(c_gen[36:41], nn_arc[4], nn_arc[3]), c_gen[42:43], Flux.relu)
    )

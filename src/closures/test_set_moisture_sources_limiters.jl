...
dTdts = FT[0, 0.002, -0.002]

inputs = Iterators.product(
    Ts,
    ρs,
    q_liqs,
    q_ices,
...
@showprogress showspeed = true for (T, ρ, q_liq, q_ice, limiter, τ_liq, τ_ice, Δt, sat_factor, sat, w, dqvdt, dTdt) in
                                   inputs
    if limiter ∈ (
    ...
    try
        ...
        (q_vap == TD.vapor_specific_humidity(q)) || error(
            "q_vap != vapor_specific_humidity(q), got $q_vap = $q_vap; vapor_specific_humidity(q) = $(TD.vapor_specific_humidity(q))",
        )

        limiter_here = if limiter === MorrisonMilbrandt2015ExponentialPartOnlyMoistureSourcesLimiter
        ...
        isfinite(new_q_liq) || begin
            (@error "new_q_liq $new_q_liq is not finite")
            (@error "Failed on T = $T; ρ = $ρ; q_liq = $q_liq; q_ice = $q_ice; limiter = $limiter; τ_liq = $τ_liq; τ_ice = $τ_ice; Δt = $Δt; sat = :$sat; sat_factor = $sat_factor; w = $w; dqvdt = $dqvdt; dTdt = $dTdt; ts = $ts; p = $p; q_vap = $q_vap; q = $q; q_eq = $q_eq; area = $area")
            continue
        end
        isfinite(new_q_ice) || begin
            (@error "new_q_ice $new_q_ice is not finite")
            (@error "Failed on T = $T; ρ = $ρ; q_liq = $q_liq; q_ice = $q_ice; limiter = $limiter; τ_liq = $τ_liq; τ_ice = $τ_ice; Δt = $Δt; sat = :$sat; sat_factor = $sat_factor; w = $w; dqvdt = $dqvdt; dTdt = $dTdt; ts = $ts; p = $p; q_vap = $q_vap; q = $q; q_eq = $q_eq; area = $area")
            continue
        end
        (new_q_liq >= FT(0)) || (
            ((abs(new_q_liq / q_liq) < 1e-8) || (q_liq < eps(FT))) || begin
                (@error "new_q_liq $new_q_liq is negative")
                (@error "Failed on T = $T; ρ = $ρ; q_liq = $q_liq; q_ice = $q_ice; limiter = $limiter; τ_liq = $τ_liq; τ_ice = $τ_ice; Δt = $Δt; sat = :$sat; sat_factor = $sat_factor; w = $w; dqvdt = $dqvdt; dTdt = $dTdt; ts = $ts; p = $p; q_vap = $q_vap; q = $q; q_eq = $q_eq; area = $area")
                continue
            end
        )
        (new_q_ice >= FT(0)) || (
            ((abs(new_q_ice / q_ice) < 1e-8) || (q_ice < eps(FT))) || begin
                (@error "new_q_ice $new_q_ice is negative")
                (@error "Failed on T = $T; ρ = $ρ; q_liq = $q_liq; q_ice = $q_ice; limiter = $limiter; τ_liq = $τ_liq; τ_ice = $τ_ice; Δt = $Δt; sat = :$sat; sat_factor = $sat_factor; w = $w; dqvdt = $dqvdt; dTdt = $dTdt; ts = $ts; p = $p; q_vap = $q_vap; q = $q; q_eq = $q_eq; area = $area")
                continue
            end
        )

        (δ_ql ≥ (-q_liq - 1e-8 * q_liq)) || (
            ((abs(new_q_liq / q_liq) < 1e-8) || (q_liq < eps(FT))) || begin
                (@error "δ_ql $δ_ql is too negative for q_liq = $q_liq")
                (@error "Failed on T = $T; ρ = $ρ; q_liq = $q_liq; q_ice = $q_ice; limiter = $limiter; τ_liq = $τ_liq; τ_ice = $τ_ice; Δt = $Δt; sat = :$sat; sat_factor = $sat_factor; w = $w; dqvdt = $dqvdt; dTdt = $dTdt; ts = $ts; p = $p; q_vap = $q_vap; q = $q; q_eq = $q_eq; area = $area")
                continue
            end
        )
        (δ_qi ≥ (-q_ice - 1e-8 * q_ice)) || (
            ((abs(new_q_ice / q_ice) < 1e-8) || (q_ice < eps(FT))) || begin
                (@error "δ_qi $δ_qi is too negative for q_ice = $q_ice")
                (@error "Failed on T = $T; ρ = $ρ; q_liq = $q_liq; q_ice = $q_ice; limiter = $limiter; τ_liq = $τ_liq; τ_ice = $τ_ice; Δt = $Δt; sat = :$sat; sat_factor = $sat_factor; w = $w; dqvdt = $dqvdt; dTdt = $dTdt; ts = $ts; p = $p; q_vap = $q_vap; q = $q; q_eq = $q_eq; area = $area")
                continue
            end
        )
        ...
        if occursin("Timescales", sprint(showerror, e)) &&
           occursin("are too fast", sprint(showerror, e)) &&
           ((τ_liq < eps(FT)) || (τ_ice < eps(FT))) # ignore timescales too fast error in Standard() 

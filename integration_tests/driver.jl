...
if run_from_command_line
    ...
    ArgParse.@add_arg_table s begin
        ...
        "--area_limiter_power"     # Choose area limiter power
        "--entr_dim_scale"         # Choose entrainment dim scale
...
area_limiter_power = run_from_command_line ? parsed_args["area_limiter_power"] : nothing
entr_dim_scale     = run_from_command_line ? parsed_args["entr_dim_scale"] : nothing

area_limiter_power ≠ nothing && (area_limiter_power = parse(Float64, area_limiter_power))
...
uuid_suffix(s::Number) = "_" * "$s"
uuid_suffix(::Nothing) = ""
suffix = uuid_suffix(micro)
suffix *= uuid_suffix(entr)
suffix *= uuid_suffix(stoch_entr)
suffix *= uuid_suffix(area_limiter_power)
suffix *= uuid_suffix(entr_dim_scale)
...
area_limiter_power ≠ nothing && (namelist["turbulence"]["EDMF_PrognosticTKE"]["area_limiter_power"] = area_limiter_power)
entr_dim_scale     ≠ nothing && (namelist["turbulence"]["EDMF_PrognosticTKE"]["entr_dim_scale"] = entr_dim_scale)
#! format: on

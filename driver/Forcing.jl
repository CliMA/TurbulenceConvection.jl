...
function initialize(forcing::ForcingBase{ForcingSOCRATES_RF09_obs}, grid, state, param_set, Dat::SOCRATES_RF09_obsData) # added param_set so we can calculate stuff....
    FT = eltype(param_set)
    aux_gm = TC.center_aux_grid_mean(state)
    forcing_funcs = create_forcing_funcs(forcing, grid, state, param_set; Dat) # named tuple

...
function affect_io!(integrator)
    ...
    for inds in TC.iterate_columns(prog.cent)
        ...
        compute_diagnostics!(edmf, precip_model, param_set, grid, state, diag_col, surf, t, calibrate_io)

        NC.NCDataset(stats.nc_filename, "a") do ds
            export_diagnostic_timeseries!(ds, grid, state, surf, diag_col, surf_params, t, calibrate_io)
            export_diagnostic_fields!(ds, state, io_nt.aux)
            export_diagnostic_fields!(ds, diag_col, io_nt.diagnostics)
        end

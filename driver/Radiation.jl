...
function update_radiation(self::RadiationBase{RadiationDYCOMS_RF01}, state, t::Real, param_set)
    ...
    rprob = SciMLBase.ODEProblem{false, SciMLBase.FullSpecialize}(rintegrand, 0.0, rz_span, params; dt = Δz) # false means not in place
    rsol = SciMLBase.solve(rprob, OrdinaryDiffEqTsit5.Tsit5(), reltol = 1e-12, abstol = 1e-12)
    q_0 = rsol.(vec(grid.zf.z))

    prob = SciMLBase.ODEProblem{false, SciMLBase.FullSpecialize}(integrand, 0.0, z_span, params; dt = Δz) # false means not in place

...
struct RRTMGPModel{Optics, No_LW, No_SW, Update_BL, ST, LT, PT, OT, MT}
    solver::ST
    lookups::LT
    param_set::PT
    max_threads::Int
...
function coordinate_info(pass, DA, add_boundary, extension, args...)
    ...
    nlay_extension, extension_coords = extension_info(pass, DA, FT, ncol, extension)
    nlay_boundary = add_boundary ? 1 : 0
    return (
        FT,
        ncol,
        nlay_domain + nlay_extension + nlay_boundary,
        (;
...
domain_info(pass, DA, FT::Type{<:AbstractFloat}, nlay::Int, ncol::Int = 1) = (FT, ncol, nlay, NoCoordsProvided{pass}())
function domain_info(pass, DA, z_lev::AbstractArray{<:AbstractFloat, 2})
    zs = (; lay = DA((z_lev[1:(end - 1), :] .+ z_lev[2:end, :]) ./ 2), lev = DA(z_lev))
    return (eltype(z_lev), size(z_lev, 2), size(z_lev, 1) - 1, VertCoordsProvided{pass, typeof(zs)}(zs))
end
...
function extension_info(pass, DA, FT, ncol, z_lev::AbstractArray{<:AbstractFloat, 2})
    if eltype(z_lev) != FT
...
function rrtmgp_artifact(subfolder, file_name)
    ...
    artifacts_file = joinpath(dirname(dirname(pathof(RRTMGP))), "Artifacts.toml")
    data_folder = joinpath(Pkg.Artifacts.ensure_artifact_installed(artifact_name, artifacts_file), artifact_name)
    return Dataset(joinpath(data_folder, subfolder, file_name), "r")
end

function set_and_record_array!(array, name, vertical_symbol, rec, coords, init_dict)
    symbol = Symbol(name)
    ...
    if ext_symbol in keys(init_dict)
        if vertical_symbol == :NA
            throw(ArgumentError("got unsupported keyword argument $ext_symbol",))
        end
...
function set_arrays!(array, value, ext_value, vertical_symbol, coords)
    ...
    elseif coords.nlay_ghost == 0
        ...
        if coords.nlay_ghost != coords.nlay_boundary
            ext_array = @view( array[(end - coords.nlay_ghost + 1):(end - coords.nlay_boundary), :])
...
function set_array!(array, value::AbstractArray{<:Real, 2}, args...)
    ...
    elseif size(value) == (1, size(array, 2))
        ...
        throw(ArgumentError(string("expected array of size $(size(array)); ", "received array of size $(size(value))")))
    end
end
set_array!(array, value::Function, args...) = set_array_fun!(array, value, args...)
...
set_array_fun!(array, f, coords::AbstractCoordsProvided{true}) =
    ndims(array) == 1 ? array .= f.(1:size(array, 1)) : array .= f.(1:size(array, 1), (1:size(array, 2))')
set_array_fun!(array, f, coords::NoCoordsProvided{false}, vertical_symbol) = array .= f.(1:size(array, 1))
...
uniform_z_p(t, t1, t2, p1, p2) = t1 == t2 ? sqrt(p1 * p2) : # p1 * (p2 / p1)^0.5
    p1 * (p2 / p1)^(log(t / t1) / log(t2 / t1))
best_fit_p(t, t1, t2, p1, p2, precomputed_z_value) =
    t1 == t2 ? p1 * (p2 / p1)^precomputed_z_value : p1 * (p2 / p1)^(log(t / t1) / log(t2 / t1))
...
function RRTMGPModel(
    ...
    if !(optics in optics_modes)
        throw(ArgumentError(string("keyword argument optics must be set to ", join(optics_modes, ", ", ", or "))))
    end
    level_modes = (:none, :average, :geometric_mean, :uniform_z, :uniform_p, :best_fit)
    ...
    if disable_longwave && disable_shortwave
        throw(ArgumentError("either longwave or shortwave fluxes must be enabled",))
    end
    ...
    FT, ncol, nlay, coords =
        coordinate_info(pass_column_to_function, DA, add_isothermal_boundary_layer, extension, args...)
    ...
    if disable_longwave
        ...
        if optics == :gray
            ...
            lookup_lw, idx_gases = RRTMGP.LookUpTables.LookUpLW(ds_lw, Int, FT, DA)
            close(ds_lw)
            ...
            if optics == :all
                ...
                lookup_lw_cld = RRTMGP.LookUpTables.LookUpCld(ds_lw_cld, Int, FT, DA, !use_pade_method)
                close(ds_lw_cld)
                lookups = (; lookups..., lookup_lw_cld)
            end
        end

        src_lw = RRTMGP.Sources.source_func_longwave(FT, ncol, nlay, op_symb, DA)
        flux_lw = RRTMGP.Fluxes.FluxLW(ncol, nlay, FT, DA)
        fluxb_lw = optics == :gray ? nothing : RRTMGP.Fluxes.FluxLW(ncol, nlay, FT, DA)
        ...
        if optics == :all_with_clear
            flux_lw_clear = RRTMGP.Fluxes.FluxLW(ncol, nlay + 1 - coords.nlay_ghost, FT, DA)
        ...
        if :top_of_atmosphere_dn_lw_flux in keys(init_dict)
            ...
            set_and_record_array!(transpose(inc_flux), "top_of_atmosphere_dn_lw_flux", :NA, tuple...)
        else
            inc_flux = nothing
        end
    ...
    if disable_shortwave
        ...
        if optics == :gray
            ...
            lookup_sw, idx_gases = RRTMGP.LookUpTables.LookUpSW(ds_sw, Int, FT, DA)
            close(ds_sw)
            ...
            if optics == :all
                ...
                lookup_sw_cld = RRTMGP.LookUpTables.LookUpCld(ds_sw_cld, Int, FT, DA, !use_pade_method)
                close(ds_sw_cld)
                lookups = (; lookups..., lookup_sw_cld)
            end
        end

        src_sw = RRTMGP.Sources.source_func_shortwave(FT, ncol, nlay, op_symb, DA)
        flux_sw = RRTMGP.Fluxes.FluxSW(ncol, nlay, FT, DA)
        fluxb_sw = optics == :gray ? nothing : RRTMGP.Fluxes.FluxSW(ncol, nlay, FT, DA)
        ...
        if optics == :all_with_clear
            flux_lw_clear = RRTMGP.Fluxes.FluxSW(ncol, nlay + 1 - coords.nlay_ghost, FT, DA)
        ...
        set_and_record_array!(toa_flux, "weighted_irradiance", :NA, tuple...)
        set_and_record_array!(sfc_alb_direct, "dir_sw_surface_albedo", :NA, tuple...)
        set_and_record_array!(sfc_alb_diffuse, "dif_sw_surface_albedo", :NA, tuple...)
        if :top_of_atmosphere_dif_dn_sw_flux in keys(init_dict)
            ...
            set_and_record_array!(transpose(inc_flux_diffuse), "top_of_atmosphere_dif_dn_sw_flux", :NA, tuple...)
        else
            inc_flux_diffuse = nothing
        end
        bcs_sw = RRTMGP.BCs.SwBCs(zenith, toa_flux, sfc_alb_direct, inc_flux_diffuse, sfc_alb_diffuse)
    end
    ...
    implicit_lays = !(:pressure in keys(init_dict) && :temperature in keys(init_dict))
    implicit_levs = !(:level_pressure in keys(init_dict) && :level_temperature in keys(init_dict))
    if !implicit_lays
    ...
    if optics == :gray
        ...
        gas_names = filter(gas_name -> !(gas_name in ("h2o", "h2o_frgn", "h2o_self", "o3")), keys(idx_gases))
        ...
        if use_global_means
            vmr = RRTMGP.Vmrs.VmrGM(DA{FT}(undef, nlay, ncol), DA{FT}(undef, nlay, ncol), DA{FT}(undef, ngas))
            vmr.vmr .= zero(FT)
            ...
            for gas_name in gas_names
                set_and_record_array!(@view(vmr.vmr[idx_gases[gas_name]]), vmr_str * gas_name, :NA, tuple...)
            end
        else
            vmr = RRTMGP.Vmrs.Vmr(DA{FT}(undef, nlay, ncol, ngas))
            for gas_name in ["h2o", "o3", gas_names...]
                set_and_record_array!(@view(vmr.vmr[:, :, idx_gases[gas_name]]), vmr_str * gas_name, :lay, tuple...)
            end
        end
        ...
        if optics == :clear
            ...
            set_and_record_array!(cld_r_eff_liq, "cloud_liquid_effective_radius", :lay, tuple...)
            set_and_record_array!(cld_r_eff_ice, "cloud_ice_effective_radius", :lay, tuple...)
            set_and_record_array!(cld_path_liq, "cloud_liquid_water_path", :lay, tuple...)
            set_and_record_array!(cld_path_ice, "cloud_ice_water_path", :lay, tuple...)
            ...
            if !(ice_rgh in (1, 2, 3))
                throw(ArgumentError("keyword argument ice_roughness must be 1, 2, or 3",))
            end
        end
    ...
    if length(init_dict) > 0
        throw(ArgumentError(string(
            ...
            join(keys(init_dict), ", ", length(init_dict) == 2 ? " and " : ", and "),
        )))
    end

    if implicit_levs
        tuple = (level_computation, p_lay, p_lev, t_lay, t_lev, coords.domain.zs.lay, coords.domain.zs.lev, param_set)
        bottom_tuple = (use_ideal_coefs_for_bottom_level ? :ideal_coefs : level_computation, tuple[2:end]...)
        ...
        if implicit_lays
            ...
            level_olators = (level_olator(true, 1:i, 1:i, 2:(i + 1), tuple...),)
        else
            level_olators = ()
        end
        if use_ideal_coefs_for_bottom_level
            @warn string("the bottom level will not be automatically computed when ", "`level_computation = :none`")
        end
    end
...
compute_boundary_layer!(::RRTMGPModel{Optics, No_LW, No_SW, false}) where {Optics, No_LW, No_SW} = nothing
compute_boundary_layer!(model::RRTMGPModel) = compute_boundary_layer!(model.solver.as, model.lookups)
...
get_p_min(as::RRTMGP.AtmosphericStates.GrayAtmosphericState, lookups) = zero(eltype(as.p_lay))
get_p_min(as::RRTMGP.AtmosphericStates.AtmosphericState, lookups) = lookups[1].p_ref_min
compute_boundary_layer_vmr!(as::RRTMGP.AtmosphericStates.GrayAtmosphericState) = nothing
compute_boundary_layer_vmr!(as::RRTMGP.AtmosphericStates.AtmosphericState) = compute_boundary_layer_vmr!(as.vmr)
...
get_vmr_h2o(vmr::RRTMGP.Vmrs.Vmr, idx_gases) = @view vmr.vmr[:, :, idx_gases["h2o"]]

compute_lw_fluxes!(::RRTMGPModel) = nothing
compute_lw_fluxes!(model::RRTMGPModel{:gray, false}) = RRTMGP.RTESolver.solve_lw!(model.solver, model.max_threads)
compute_lw_fluxes!(model::RRTMGPModel{:clear, false}) =
    RRTMGP.RTESolver.solve_lw!(model.solver, model.max_threads, model.lookups.lookup_lw)
compute_lw_fluxes!(model::RRTMGPModel{:all, false}) =
    RRTMGP.RTESolver.solve_lw!(model.solver, model.max_threads, model.lookups.lookup_lw, model.lookups.lookup_lw_cld)
...
function compute_optical_props_kernel_lw!(
    ...
    @inbounds op.τ[glaycol...] =
        α *
        τ0_lw[gcol] *
        (f / p0 + FT(4) * (FT(1) - f) / p0 * (p_lay[glaycol...] / p0)^3) *
        (p_lev[glaycol...] - p_lev[glay + 1, gcol])
...
compute_sw_fluxes!(model::RRTMGPModel{:clear, No_LW, false}) where {No_LW} =
    RRTMGP.RTESolver.solve_sw!(model.solver, model.max_threads, model.lookups.lookup_sw)
compute_sw_fluxes!(model::RRTMGPModel{:all, No_LW, false}) where {No_LW} =
    RRTMGP.RTESolver.solve_sw!(model.solver, model.max_threads, model.lookups.lookup_sw, model.lookups.lookup_sw_cld)
function compute_sw_fluxes!(model::RRTMGPModel{:all_with_clear, No_LW, false}) where {No_LW}
    RRTMGP.RTESolver.solve_sw!(model.solver, model.max_threads, model.lookups.lookup_sw)
...
function compute_fluxes!(model::RRTMGPModel)
    ...
    model.solver.as.p_lay .= max.(model.solver.as.p_lay, get_p_min(model.solver.as, model.lookups))
    model.solver.as.p_lev .= max.(model.solver.as.p_lev, get_p_min(model.solver.as, model.lookups))
...
function get_var(string, ds_input, nsite, nexpt, ncol)
    var = ds_input[string]
    arr = Array(var)
    if ndims(arr) == 3
